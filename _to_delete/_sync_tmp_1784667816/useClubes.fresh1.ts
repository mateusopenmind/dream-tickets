  ...
  return useMutation({
    mutationFn: async (a: AssinaturaInput) => {
      ...
      if (bonusValidos.length) {
        ...
        ).select("id, pontos, frequencia_meses, repeticoes, primeira_entrada");
        if (error) throw error;
        bonusSalvos = data ?? [];
      }
      ...
      for (const b of bonusSalvos) {
        ...
        for (let k = 0; k < reps; k++) {
          movimentos.push({
            ...
            pontos: Math.round(b.pontos || 0),
            origem: freq === 0 ? "Bônus único" : freq === 1 ? "Bônus mensal" : `Bônus a cada ${freq} meses`,
...
export function useConfirmarMovimentoEditado() {
  ...
  return useMutation({
    mutationFn: async (p: {
      ...
      plano?: string | null; cartao_codigo?: string | null; cartao_final?: string | null; motivo?: string | null;
    }) => {
      const upd: any = {
...
export function useAdicionarMovimento() {
  ...
  return useMutation({
    mutationFn: async (m: {
      ...
      pontos: number; descricao?: string | null; origem?: string | null;
    }) => {
      const { error } = await sb.from("estoque_movimentos").insert({
        ...
        pontos: Math.round(m.pontos || 0), descricao: m.descricao ?? null, origem: m.origem ?? null,
...
export function useProgramasComEstoque() {
  return useQuery({ queryKey: ["programas-com-estoque"], queryFn: async () => {
    const { data, error } = await sb
      .from("programas")
      .select("id, nome, observacao, programa_estoque_id, programas_estoque(nome)")
      .order("nome");
    if (error) throw error;
    return (data ?? []) as any[];
  }});
