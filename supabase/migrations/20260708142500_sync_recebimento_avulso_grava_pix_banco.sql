  ...
  IF v_qtd > 0 THEN
    SELECT string_agg(
      format('%s/%s - %s - R$ %s - %s%s',
        ...
        replace(replace(replace(to_char(valor, 'FM999G999G990D00'), ',', '§'), '.', ','), '§', '.'),
        upper(status),
        CASE WHEN banco IS NOT NULL AND banco <> '' THEN ' (' || banco || ')' ELSE '' END
    ...
    -- Banco "representativo" da emissão: prioriza o banco de uma parcela já recebida (mais recente),
    -- senão cai para o banco escolhido no lançamento (mesmo valor em todas as parcelas da leva).
    SELECT banco INTO v_banco
      FROM public.recebimentos_avulsos
      WHERE tabela_origem = v_tabela AND emissao_id = v_emissao_id AND banco IS NOT NULL AND banco <> ''
      ORDER BY (status = 'recebido') DESC, COALESCE(data_recebimento, data_prevista) DESC, numero_parcela DESC
      LIMIT 1;
  ELSE
    v_obs := NULL;
    v_banco := NULL;
    ...
    IF v_status IS NOT NULL AND v_status <> 'CANCELADO' THEN
      ...
      ELSE
        UPDATE public.emissoes SET
          ...
          pix_banco = (CASE WHEN v_qtd > 0 THEN v_banco WHEN forma_cobranca = 'avulso' THEN NULL ELSE pix_banco END),
