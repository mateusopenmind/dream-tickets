...
// chave e o pix_txid (aleatorio, um por cobranca). O id_emissao antigo ainda passa
// ate LEGADO_ATE, so para nao quebrar os links ja enviados no WhatsApp.
//
// ATENCAO: enquanto essa data nao vence, a chave sequencial antiga continua valendo e a
// enumeracao ainda e possivel. O rate limit por IP e o que segura essa janela.
//
...
// txid no padrao Bacen: 26 a 35 alfanumericos. O Sicredi gera 32 hex minusculos, mas o Sicoob
// gera 35 com letras maiusculas fora do a-f (ex.: MZO1KD58PMQ0UL042473701789340824366). A regex
// antiga so aceitava 32 hex, entao todo link /pix/<txid> do Sicoob caia no 404 (13/09/2026).
// id_emissao legado (MO000001) tem 8 caracteres e nao colide; UUID tem hifen e cai na RE_UUID.
const RE_TXID = /^[A-Za-z0-9]{26,35}$/;
...
Deno.serve(async (req) => {
  ...
  if (!id) return json(NAO_DISPONIVEL, 404);
