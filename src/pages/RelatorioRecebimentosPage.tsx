...
export default function RelatorioRecebimentosPage() {
  ...
  const exportar = () => {
    const colunas: ColunaExcel<any>[] = [
      { header: "Emissão", key: "id_emissao", largura: 14 },
      { header: "Tipo", key: "tabela_origem", largura: 14 },
      { header: "Cód. Cliente", key: "cliente_codigo", largura: 12 },
      { header: "Cliente", key: "cliente_nome", largura: 28 },
      { header: "Parcela", key: "parcela_txt", largura: 10 },
      { header: "Status", key: "status", largura: 12 },
      { header: "Data Prevista", key: "data_prevista", tipo: "data", largura: 14 },
      { header: "Data Recebimento", key: "data_recebimento", tipo: "data", largura: 17 },
      { header: "Banco", key: "banco", largura: 18 },
      { header: "Valor", key: "valor", tipo: "moeda", largura: 14 },
    ];
    const linhas = filtered.map((r) => ({ ...r, parcela_txt: `${r.numero_parcela}/${r.total_parcelas}` }));
    exportarExcel(`relatorio-recebimentos-${format(new Date(), "yyyy-MM-dd")}`, colunas, linhas, "Recebimentos");
  };
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <p className="text-xs text-muted-foreground pb-2">{contar(filtered.length, "parcela", "parcelas")} no filtro.</p>
        </>
      )}
    </div>
  );
