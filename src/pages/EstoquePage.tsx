...
const fmtMesRef = (iso: string | null) => (iso && iso.length >= 7 ? `${iso.slice(5, 7)}/${iso.slice(0, 4)}` : "—");
const isoHoje = () => {
  const n = new Date();
...
export default function EstoquePage() {
  ...
  const confTable = (list: any[], vazio = "Nada pendente") => (
    <Table>
      <TableHeader>
        <TableRow>
          <TableHead>Data prevista</TableHead><TableHead>Mês referência</TableHead><TableHead>Conta</TableHead><TableHead>Programa</TableHead>
          ...
          {confMov && (
            <div className="grid gap-3 py-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {!ehBonusConf && (
                  <div className="grid gap-1"><Label>Valor (R$)</Label><Input type="number" min={0} step="0.01" value={confForm.valor} onChange={(e) => setConfForm((f) => ({ ...f, valor: e.target.value }))} /></div>
                )}
              </div>
              {!ehBonusConf && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="grid gap-1"><Label>Final cartão</Label><Input maxLength={4} value={confForm.cartao_final} onChange={(e) => setConfForm((f) => ({ ...f, cartao_final: e.target.value.replace(/\D/g, "").slice(0, 4) }))} /></div>
                </div>
              )}
