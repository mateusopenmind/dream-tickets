...
export default function RelatorioSaldosPage() {
  ...
  const fmtSeguro = (valor: string, mascara: string) => {
    const d = new Date(valor);
    return isNaN(d.getTime()) ? "" : format(d, mascara);
  };
  const dataSaldo = (l: LinhaSaldo): string => {
    if (l.email_em) return fmtSeguro(l.email_em, "dd/MM/yyyy HH:mm") || "—";
    if (l.atualizado_em) return fmtSeguro(l.atualizado_em, "dd/MM/yyyy HH:mm") || "—";
    if (l.data_ref) return fmtSeguro(String(l.data_ref).slice(0, 10) + "T00:00:00", "dd/MM/yyyy") || "—";
    return "—";
  };

  const exportar = () => {
    const colunas: ColunaExcel<any>[] = [
      { header: "Código", key: "conta_codigo", largura: 12 },
      { header: "Conta", key: "conta_nome", largura: 28 },
      { header: "Tipo", key: "conta_tipo", largura: 14 },
      { header: "Programa", key: "programa", largura: 20 },
      { header: "Status", key: "status_txt", largura: 10 },
      { header: "Saldo", key: "saldo", tipo: "inteiro", largura: 14 },
      { header: "Data do saldo", key: "data_saldo_txt", largura: 18 },
    ];
    const linhasXls = sorted.map((l: LinhaSaldo) => ({
      ...l,
      status_txt: l.ativo ? "Ativo" : "Inativo",
      data_saldo_txt: dataSaldo(l) === "—" ? "" : dataSaldo(l),
    }));
    exportarExcel(`saldos-milhas-${format(new Date(), "yyyy-MM-dd")}`, colunas, linhasXls, "Saldos");
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" onClick={exportar} disabled={sorted.length === 0}>
      ...
      {inativosComSaldo > 0 && (
        <div className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900 flex items-center gap-2">
          ...
          {contar(inativosComSaldo, "programa inativo", "programas inativos")} ainda com saldo &mdash; {fmt(totalInativas)} milhas paradas (conta não participa mais, mas o saldo segue gravado).
        </div>
      )}
      ...
      <Card className="overflow-x-auto">
        ...
        {sorted.length > 0 && (
          <div className="flex flex-wrap justify-between items-center gap-x-6 gap-y-1 border-t px-4 py-3 text-sm">
            <span className="text-muted-foreground">{contar(sorted.length, "registro", "registros")}</span>
