...
export default function RelatorioSaldosPage() {
  ...
  const { data: linhas, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<LinhaSaldo[]> => {
      const { data, error } = await supabase
        ...
        .select("id, saldo_milhas, saldo_data_ref, saldo_atualizado_em, ativo, contas(codigo, nome, tipo), programas(nome)")
        .limit(5000);
      ...
      return (data ?? []).map((r: any) => ({
        ...
        programa: (r.programas as any)?.nome ?? "",
        ativo: r.ativo ?? true,
  ...
  // Data do saldo: prioriza a data de referência ("Saldo em DD/MM" do e-mail / coluna da planilha).
  // Cai para o timestamp de gravação só quando não há data de referência (registros antigos).
