...
export default function ComprasPage() {
  ...
  const { query, setQuery, filtered } = useSearch<any>(rows, ["id_compra", "_conta", "_estoque", "forma_pagamento", "descricao", "operacao"]);
    ...
    <div className="space-y-4">
      ...
      <Card className="overflow-hidden">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Código</TableHead><TableHead>Data</TableHead><TableHead>Conta</TableHead><TableHead>Programa</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow><TableCell colSpan={12} className="text-center text-muted-foreground py-6">Carregando...</TableCell></TableRow>
            ) : filtered.length === 0 ? (
              <TableRow><TableCell colSpan={12} className="text-center text-muted-foreground py-6">Nenhuma compra lançada</TableCell></TableRow>
            ) : filtered.map((c: any) => (
              <TableRow key={c.id}>
                <TableCell className="py-2 font-mono">{c.id_compra || "—"}</TableCell>
                <TableCell className="py-2">{fmtData(c.data)}</TableCell>
                <TableCell className="py-2"><span className="font-mono font-semibold text-base">{c.contas?.codigo}</span></TableCell>
                ...
                <TableCell className="py-2 text-muted-foreground">{c.meio_pagamento === "pix" ? `Pix${c.bancos?.nome ? " · " + c.bancos.nome : ""}` : (c.forma_pagamento || "—")}</TableCell>
