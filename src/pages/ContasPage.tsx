...
import { useContas, useUpsertConta, useDeleteConta, useProgramas, useContaProgramas } from "@/hooks/useData";
...
// programas marcados de uma conta: { [programa_id]: { ativo, saldo } }
...
export default function ContasPage() {
  ...
  const saldos = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        ...
        .select("conta_id, saldo_milhas, ativo, programas(nome)")
        .gt("saldo_milhas", 0)
        .limit(5000);
      const map: Record<string, { nome: string; saldo: number; ativo: boolean }[]> = {};
      (data ?? []).forEach((r: any) => {
        (map[r.conta_id] ||= []).push({ nome: (r.programas as any)?.nome ?? "", saldo: r.saldo_milhas ?? 0, ativo: r.ativo ?? true });
      });
      return map;
    },
  });
  ...
  const openEdit = async (c: any) => {
    ...
    const { data } = await supabase.from("conta_programas").select("programa_id, saldo_milhas, ativo").eq("conta_id", c.id);
    const m: ProgMap = {};
    (data ?? []).forEach((r: any) => { m[r.programa_id] = { ativo: r.ativo ?? true, saldo: r.saldo_milhas ?? 0 }; });
    setProgMap(m);
    setOpen(true);
  };
  ...
  const removerProg = async (id: string) => {
    const nomePrograma = (programas ?? []).find((p: any) => p.id === id)?.nome;
    if (editing?.id && nomePrograma) {
      const { count } = await supabase
        .from("emissoes")
        .select("id", { count: "exact", head: true })
        .eq("conta_id", editing.id)
        .eq("programa", nomePrograma);
      if ((count ?? 0) > 0) {
        toast.error(`Não é possível remover "${nomePrograma}": já existe emissão desta conta nesse programa. Desmarque para Inativar.`);
        return;
      }
    }
  ...
  const progsDisponiveis = (programas ?? []).filter((p: any) => !progMap[p.id]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (contaId) {
        ...
        const { data: atuais } = await supabase.from("conta_programas").select("id, programa_id").eq("conta_id", contaId);
        for (const r of (atuais ?? [])) {
          if (!idsNoForm.includes((r as any).programa_id)) {
        ...
        for (const [pid, v] of Object.entries(progMap)) {
          await supabase.from("conta_programas").upsert(
            { conta_id: contaId, programa_id: pid, ativo: v.ativo, saldo_milhas: Math.round(v.saldo || 0), saldo_atualizado_em: new Date().toISOString() },
            { onConflict: "conta_id,programa_id" }
          );
        }
      }
      qc.invalidateQueries({ queryKey: ["conta-programas-todos"] });
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await remove.mutateAsync(id); qc.invalidateQueries({ queryKey: ["conta-programas-todos"] }); toast.success("Conta removida!"); }
              ...
              {Object.keys(progMap).length === 0 ? (
                ...
                <div className="rounded-md border">
                  ...
                  <div className="divide-y">
                    {(programas ?? []).filter((p: any) => progMap[p.id]).map((p: any) => {
