...
const emptyForm = { codigo: "", nome: "", nascimento: "", cpf: "", fone: "", email: "", tipo: "Própria", pix_envio: "", pix_titular: "", numero_smiles: "" };
...
export default function ContasPage() {
  ...
  const openEdit = async (c: any) => {
    ...
    setForm({
      ...
      fone: c.fone ?? "", email: c.email ?? "", tipo: c.tipo ?? "Própria", pix_envio: c.pix_envio ?? "", pix_titular: c.pix_titular ?? "",
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-3 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1"><Label>Titular da chave (se for outra pessoa)</Label><Input value={form.pix_titular} onChange={(e) => set("pix_titular", e.target.value)} placeholder="Deixe vazio quando a chave for da própria pessoa" /></div>
                    ...
                    {(programasEstoque ?? []).filter((p: any) => progMap[p.id]).map((p: any) => {
                      ...
                      return (
                        <div key={p.id} className="grid grid-cols-[1fr_72px_120px_36px] items-center gap-2 px-3 py-1.5">
                          ...
                          {isSuper ? (
                            <NumericInput value={saldo} onChange={(n) => setSaldoProg(p.id, n)} className="text-right" />
                          ) : (
