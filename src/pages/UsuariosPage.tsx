...
export default function UsuariosPage() {
  ...
  const telasLiberaveis = (telasCatalogo.data ?? []).filter(
    (t: any) => isSuper || (minhasTelas?.has(t.chave) ?? false)
  );
      ...
      <Dialog open={telasOpen} onOpenChange={setTelasOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2 max-h-[60vh] overflow-y-auto">
            {[1, 2, 3].map((fase) => {
              const doFase = telasLiberaveis.filter((t: any) => t.fase === fase);
              if (doFase.length === 0) return null;
              return (
                <div key={fase} className="space-y-2">
                  <p className="text-sm font-semibold text-muted-foreground">Fase {fase}</p>
                  {doFase.map((t: any) => (
                    <label key={t.id} className="flex items-center gap-2 text-sm">
                      ...
                      />
                      <span className={!t.pronta ? "text-muted-foreground" : ""}>
                        {t.nome} {!t.pronta && <span className="text-xs">(em breve)</span>}
                      </span>
                    </label>
                  ))}
                </div>
