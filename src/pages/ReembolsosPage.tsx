...
// Texto da situação (Cobrança) — mesma regra do badge, usado também na busca.
const textoCobranca = (r: any): string => {
  if (r.sentido === "cobrar") {
    if (r.status_pix === "PAGO" || r.status_pix === "CANCELADO" || r.status_pix === "EM ABERTO") return r.status_pix;
    // Liquidacao zerada: nao ha o que cobrar, o sistema ja deu baixa.
    if (r.pago) return "PAGO";
    return "A COBRAR";
  }
  return r.pago ? "PAGO" : "A REEMBOLSAR";
};
...
export default function ReembolsosPage() {
  ...
  // Campos _xxx: valores derivados/formatados como aparecem na tabela, p/ a busca cobrir todas as colunas.
  const visiveis = useMemo(() => (reembolsos ?? [])
    .filter((r: any) => r.status_pix !== "CANCELADO")
    .map((r: any) => ({
      ...r,
      _dataHora: fmtDataHora(r.created_at),
      _cliente: r.clientes ? `${r.clientes.codigo ?? ""} ${r.clientes.nome_fantasia ?? ""}`.trim() : "",
      _milhas: fmtMilhas(r.total_milhas),
      _total: fmtMoeda(r.emissao_total),
      _cobranca: textoCobranca(r),
      _tipo: rotuloReembolso(r.tipo, r.pax_qtd, r.pax_total),
      _motivo: MOTIVO_LABEL[r.motivo] ?? r.motivo ?? "",
      _reembCliente: fmtMoeda(r.total_cliente),
      _reembDream: fmtMoeda(r.total_dream),
    })), [reembolsos]);
  const { query, setQuery, filtered } = useSearch(visiveis, [
    "reembolso_id", "id_emissao", "localizador", "programa", "operacao", "conta", "tipo", "motivo",
    "_dataHora", "_cliente", "_milhas", "_total", "_cobranca", "_tipo", "_motivo", "_reembCliente", "_reembDream",
  ]);
  ...
  const statusBadge = (r: any) => {
    if (r.sentido === "cobrar") {
      ...
      // Liquidacao zerada: nao ha o que cobrar, o sistema ja deu baixa.
      if (r.pago) return <Badge className="bg-success text-success-foreground">PAGO</Badge>;
  ...
  return (
    <div className="space-y-4">
      ...
      <SearchBar value={query} onChange={setQuery} placeholder="Buscar em toda a tabela..." />
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Card className="hidden lg:block overflow-x-auto">
            <Table className="text-xs [&_th]:px-2 [&_td]:px-2 [&_th]:whitespace-nowrap">
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {paged.map((r: any) => (
                  <TableRow key={r.id}>
                    <TableCell className="py-2 font-mono whitespace-nowrap">{r.reembolso_id || "—"}</TableCell>
                    <TableCell className="py-2 whitespace-nowrap">{fmtDataHora(r.created_at)}</TableCell>
                    <TableCell className="py-2 font-mono font-semibold">{r.localizador || "—"}</TableCell>
                    <TableCell className="py-2">{r.programa || "—"}</TableCell>
                    <TableCell className="py-2"><span className="block max-w-[90px] truncate" title={r.operacao || ""}>{r.operacao || "—"}</span></TableCell>
                    <TableCell className="py-2 font-mono">{r.clientes?.codigo || "—"}</TableCell>
                    <TableCell className="py-2 font-mono">{r.conta || "—"}</TableCell>
                    ...
                    <TableCell className="py-2 whitespace-nowrap">{rotuloReembolso(r.tipo, r.pax_qtd, r.pax_total)}</TableCell>
                    <TableCell className="py-2"><span className="block max-w-[120px] truncate" title={MOTIVO_LABEL[r.motivo] ?? r.motivo ?? ""}>{MOTIVO_LABEL[r.motivo] ?? r.motivo ?? "—"}</span></TableCell>
            ...
            {paged.map((r: any) => (
              <Card key={r.id} className="p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                  ...
                  <span>Tipo: <strong>{rotuloReembolso(r.tipo, r.pax_qtd, r.pax_total)}</strong></span>
