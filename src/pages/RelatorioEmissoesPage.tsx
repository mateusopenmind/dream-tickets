...
const pct = (v: number) => (v == null || !Number(v) ? "—" : `${Number(v).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%`);
/** Formata data vinda do banco ("yyyy-mm-dd" ou timestamp) sem quebrar a tela. */
...
/** Só operações do tipo "Emissão" contam como emissão e somam PAX (regras R1/R2 do Dashboard). */
const ehEmissao = (r: Row) => r.nome_operacao === "Emissão";

/**
 * Campo de valor da emissão sempre em R$. Taxas, bagagens, assentos e outros podem ser
 * lançados em milhas — nesse caso a coluna guarda a QUANTIDADE de milhas, que só vira
 * dinheiro depois de multiplicada pelo preço do milheiro. É a mesma conversão que o
 * formulário faz para montar o Preço Total.
 */
const emReais = (valor: unknown, tipo: unknown, milheiro: unknown) =>
  tipo === "milhas" ? ((Number(valor) || 0) * (Number(milheiro) || 0)) / 1000 : Number(valor) || 0;

/**
 * Preço médio do milheiro PONDERADO pelas milhas: Σ(milhas × milheiro ÷ 1000) ÷ Σ milhas × 1000.
 * Mesma regra do Dashboard de Faturamento — antes aqui era média simples, o que inflava o
 * número (o milheiro vai de ~R$ 13 a ~R$ 115 na base, e a média simples fazia uma emissão de
 * 5 mil milhas pesar igual a uma de 500 mil).
 */
const avgMilheiro = (arr: Row[]) => {
  let valor = 0;
  let milhas = 0;
  for (const r of arr) {
    const m = Number(r.milhas_cobrado) || 0;
    const p = Number(r.preco_milheiro) || 0;
    if (m <= 0 || p <= 0) continue;
    valor += (m * p) / 1000;
    milhas += m;
  }
  return milhas > 0 ? (valor / milhas) * 1000 : 0;
};
...
export default function RelatorioEmissoesPage() {
  ...
  const { data: rawRows, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Row[]> => {
      // Os campos *_tipo vêm junto porque taxas/bagagens/assentos/outros podem ter sido
      // lançados EM MILHAS — nesse caso a coluna guarda uma quantidade de milhas, e não
      // reais. A conversão acontece no mapeamento abaixo (ver `emReais`).
      const sel = "id_emissao, owner_id, data_emissao, data_voo_ida, programa, nome_operacao, status_pix, emissor, forma_pagamento, facial, codigo_la, percentual_cb, passageiros_qtd, milhas_cobrado, preco_milheiro, milhas_real, taxas_cobrado, taxas_tipo, taxas_real, taxas_real_tipo, outros_cobrado, outros_tipo, outros_real, outros_real_tipo, bagagens_cobrado, bagagens_tipo, assentos_cobrado, assentos_tipo, preco_total, valor_recebido, clientes(codigo), contas(codigo)";
      ...
      return data.map((r: any) => ({
        ...
        facial: !!r.facial,
        codigo_la: r.codigo_la ?? "",
        percentual_cb: r.percentual_cb ?? 0,
        ...
        // Valores sempre em R$: o que foi lançado em milhas é convertido pelo milheiro,
        // igual ao Preço Total e ao Dashboard de Faturamento.
        taxas_cobrado: emReais(r.taxas_cobrado, r.taxas_tipo, r.preco_milheiro),
        taxas_real: emReais(r.taxas_real, r.taxas_real_tipo, r.preco_milheiro),
        outros_cobrado: emReais(r.outros_cobrado, r.outros_tipo, r.preco_milheiro),
        outros_real: emReais(r.outros_real, r.outros_real_tipo, r.preco_milheiro),
        bagagens_cobrado: emReais(r.bagagens_cobrado, r.bagagens_tipo, r.preco_milheiro),
        assentos_cobrado: emReais(r.assentos_cobrado, r.assentos_tipo, r.preco_milheiro),
  ...
  const grupo = (key: keyof Row) =>
    agrupar(key).map(([k, a]) => ({ nome: k, emissoes: a.filter(ehEmissao).length, milhas: sum(a, "milhas_cobrado"), milheiro: avgMilheiro(a), taxas: sum(a, "taxas_cobrado"), preco: sum(a, "preco_total") }));
  ...
  const somatorios = [
    // "Registros" é toda linha do filtro; "Emissões" segue a regra do Dashboard de
    // Faturamento e conta só as operações do tipo Emissão (pós-vendas ficam de fora).
    { l: "Registros", v: num(filtered.length) },
    { l: "Emissões", v: num(filtered.filter(ehEmissao).length) },
    { l: "Pagas / Aberto / Cancel.", v: `${pagas} / ${abertas.length} / ${canceladas}` },
    { l: "Preço Milheiro (ponderado)", v: brl(avgMilheiro(filtered)) },
    ...
    // PAX pela regra R2: só operações do tipo Emissão. Pós-venda não soma passageiro.
    { l: "Passageiros (Nº Pax)", v: num(sum(filtered.filter(ehEmissao), "passageiros_qtd")) },
  ...
  const exportar = () => {
    const colunas: ColunaExcel<Row>[] = [
      { header: "ID", key: "id", largura: 14 },
      { header: "Data", key: "data_emissao", tipo: "data", largura: 12 },
      { header: "Voo Ida", key: "data_voo_ida", tipo: "data", largura: 12 },
      { header: "Programa", key: "programa", largura: 16 },
      { header: "Operação", key: "nome_operacao", largura: 20 },
      { header: "Status", key: "status_pix", largura: 12 },
      { header: "Emissor", key: "emissor", largura: 16 },
      { header: "Operador", key: "operador", largura: 16 },
      { header: "Conta", key: "conta", largura: 10 },
      { header: "Cliente", key: "cliente", largura: 10 },
      { header: "Cartão", key: "cartao", largura: 14 },
      { header: "Código L.A", key: "codigo_la", largura: 18 },
      { header: "% Cashback", key: "percentual_cb", tipo: "decimal", largura: 12 },
      { header: "Pax", key: "passageiros_qtd", tipo: "inteiro", largura: 8 },
      { header: "Milhas cobrado", key: "milhas_cobrado", tipo: "inteiro", largura: 15 },
      { header: "Milhas real", key: "milhas_real", tipo: "inteiro", largura: 13 },
      { header: "Preço milheiro", key: "preco_milheiro", tipo: "moeda", largura: 15 },
      { header: "Taxas cobrado", key: "taxas_cobrado", tipo: "moeda", largura: 14 },
      { header: "Taxas real", key: "taxas_real", tipo: "moeda", largura: 13 },
      { header: "Outros cobrado", key: "outros_cobrado", tipo: "moeda", largura: 15 },
      { header: "Outros real", key: "outros_real", tipo: "moeda", largura: 13 },
      { header: "Bagagens cobrado", key: "bagagens_cobrado", tipo: "moeda", largura: 17 },
      { header: "Assentos cobrado", key: "assentos_cobrado", tipo: "moeda", largura: 17 },
      { header: "Preço total", key: "preco_total", tipo: "moeda", largura: 14 },
      { header: "Valor recebido", key: "valor_recebido", tipo: "moeda", largura: 15 },
      { header: "Facial", key: "facial_txt", largura: 8 },
    ];
    const linhas = filtered.map((r) => ({ ...r, facial_txt: r.facial ? "Sim" : "Não" }));
    exportarExcel(`relatorio-emissoes-${format(new Date(), "yyyy-MM-dd")}`, colunas as ColunaExcel<any>[], linhas, "Emissões");
  };
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div>
            ...
            <Card className="overflow-hidden">
              {/* O componente Table já traz um div com overflow-auto por dentro; neutralizamos
                  para que ESTE container role tanto na vertical quanto na horizontal
                  (senão a barra horizontal fica no rodapé da tabela inteira, fora da área visível). */}
              <div className="max-h-[520px] overflow-auto [&>div]:overflow-visible">
                <Table className="min-w-max">
                  <TableHeader className="sticky top-0 bg-card z-10">
                    <TableRow>
                      ...
                      <SortableHead label="Voo Ida" sortKey="data_voo_ida" activeKey={sdet.key} dir={sdet.dir} onSort={sdet.toggle} />
                      ...
                      <SortableHead label="L.A" sortKey="codigo_la" activeKey={sdet.key} dir={sdet.dir} onSort={sdet.toggle} />
                      <SortableHead label="% Cashback" sortKey="percentual_cb" activeKey={sdet.key} dir={sdet.dir} onSort={sdet.toggle} align="right" className="text-right" />
                  ...
                  <TableBody>
                    {sdet.sorted.length === 0 ? (
                      <TableRow><TableCell colSpan={18} className="text-center text-muted-foreground py-6">Nenhuma emissão com os filtros atuais.</TableCell></TableRow>
                    ) : sdet.sorted.map((r) => (
                      <TableRow key={r.id}>
                        <TableCell className="font-mono text-xs whitespace-nowrap">{r.id}</TableCell>
                        <TableCell className="whitespace-nowrap text-muted-foreground">{data(r.data_emissao)}</TableCell>
                        <TableCell className="whitespace-nowrap text-muted-foreground">{data(r.data_voo_ida)}</TableCell>
                        ...
                        <TableCell className="font-mono text-xs whitespace-nowrap">{r.codigo_la || "—"}</TableCell>
                        <TableCell className="text-right font-mono">{pct(r.percentual_cb)}</TableCell>
          ...
          <p className="text-xs text-muted-foreground pb-2">{contar(filtered.length, "emissão", "emissões")} no filtro. Use os filtros (série BR/GL, data, programa, etc.) para conferir os totais contra a planilha de origem.</p>
        </>
      )}
    </div>
  );
