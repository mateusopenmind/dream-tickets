...
export default function RecebimentosAvulsosPage() {
  ...
  const confirmarLancamento = async () => {
    ...
    try {
      ...
      toast.success(
        `${contar(parcelas.length, "parcela lançada", "parcelas lançadas")} para ${emissaoSelecionada.id_emissao}.` +
              ...
              <Fragment key={`${g.id_emissao}-${g.cliente}`}>
                <TableRow className="bg-muted/40">
                  <TableCell colSpan={5} className="py-2 font-semibold">
                    ...
                    <Badge variant="secondary" className="ml-2">{contar(g.itens.length, "parcela", "parcelas")}</Badge>
                  </TableCell>
                </TableRow>
                {g.itens.map((p) => (
                  <TableRow key={p.id}>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 py-2 pr-1">
            <div className="grid gap-2">
              ...
              <SearchSelect
                ...
                placeholder={carregandoEmissoes ? "Carregando..." : "Busque por ID, localizador, cliente..."}
                optionsLarge
                listClassName="max-h-[min(50vh,400px)]"
                options={(emissoesEmAberto ?? []).map((e) => ({
                  value: `${e.tabela_origem}:${e.id}`,
                  label: `${e.id_emissao}${e.localizador ? ` · ${e.localizador}` : ""} — ${e.cliente_codigo} ${e.cliente_nome} — saldo ${fmtReais(e.saldo_pendente)}${e.tabela_origem === "emissoes_terceirizadas" ? " (terceirizada)" : ""}`,
                  searchText: e.localizador ?? "",
                }))}
              />
              {!emissaoSelecionada && (
                <p className="text-xs text-muted-foreground">{contar((emissoesEmAberto ?? []).length, "emissão em aberto disponível", "emissões em aberto disponíveis")}. Digite o ID, o localizador, o código ou o nome do cliente para filtrar.</p>
              )}
            </div>
