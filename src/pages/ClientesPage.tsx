...
// Valida os dígitos verificadores de um CNPJ (14 dígitos, sem máscara).
function cnpjValido(digits: string) {
  ...
  const d1 = dv(digits.slice(0, 12), p1);
  const d2 = dv(digits.slice(0, 12) + d1, [6, ...p1]);
...
// número; se o número começar com 7, 8 ou 9, usa máscara de celular de 9 dígitos (5+4), senão fixo (4+4).
function formatarFone(pais: string, valor: string): string {
  const d = (valor || "").replace(/\D/g, "");
  if (pais === "BR") {
    const ddd = d.slice(0, 2);
    const sub = d.slice(2);
    const isCel = /^[789]/.test(sub);
...
export default function ClientesPage() {
  ...
  // Preenche o endereço a partir do CEP (ViaCEP — API pública gratuita). Acionado pelo botão de busca, como no CNPJ.
  const buscarCep = async () => {
    const digits = form.cep.replace(/\D/g, "");
    if (digits.length !== 8) { toast.error("Para buscar o endereço, informe um CEP com 8 dígitos."); return; }
    setBuscandoCep(true);
    try {
  ...
  const buscarReceita = async () => {
    ...
    if (!cnpjValido(digits)) { toast.error("CNPJ inválido — confira os dígitos informados."); return; }
    ...
    try {
      ...
      if (error) {
        // Lê a mensagem real devolvida pela edge function (o supabase-js só traz um erro genérico em status não-2xx).
        let msg = "Não foi possível consultar a Receita.";
        try {
          const body = await (error as any).context?.json?.();
          if (body?.error) msg = /HTTP 4\d\d/.test(body.error) ? "CNPJ não encontrado na Receita — confira o número." : body.error;
        ...
        <DialogContent className="w-[95vw] max-w-3xl">
          ...
          <div className="grid gap-4 py-2 max-h-[70vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1 col-span-2"><Label>Chave Pix</Label><Input value={form.chave_pix} onChange={(e) => set("chave_pix", e.target.value)} placeholder="CPF/CNPJ, e-mail, telefone ou chave aleatória" /><p className="text-xs text-muted-foreground">Usada para devolver valores de reembolso quando não for possível devolver pelo Pix de origem.</p></div>
            </div>
            <div className="grid grid-cols-4 gap-3">
              <div className="grid gap-1">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button type="button" variant="outline" onClick={buscarCep} disabled={buscandoCep} title="Buscar endereço pelo CEP">
