...
export default function TransferenciasPage() {
  ...
  const { query, setQuery, filtered } = useSearch<any>(rows, ["id_transferencia", "_origem", "_destino", "descricao"]);
    ...
    <div className="space-y-4">
      ...
      <Card className="overflow-hidden">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Código</TableHead><TableHead>Data</TableHead><TableHead>Origem (−)</TableHead><TableHead>Destino (+)</TableHead>
              <TableHead className="text-right">Sai da origem</TableHead><TableHead className="text-right">Bônus</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={10} className="text-center text-muted-foreground py-6">Nenhuma transferência lançada</TableCell></TableRow>
            ) : filtered.map((t: any) => (
              <TableRow key={t.id}>
                <TableCell className="py-2 font-mono">{t.id_transferencia || "—"}</TableCell>
                ...
                <TableCell className="py-2 text-right tabular-nums text-destructive">-{nf(Math.max(0, (Number(t.qtde_transferida) || 0) - (Number(t.carrinho_qtde) || 0)))}</TableCell>
                <TableCell className="py-2 text-right tabular-nums">{t.bonus ? pct(t.bonus) : "—"}</TableCell>
                <TableCell className="py-2 text-right tabular-nums">{t.carrinho ? `${pct(t.carrinho)} · ${nf(t.carrinho_qtde)}` : "—"}</TableCell>
