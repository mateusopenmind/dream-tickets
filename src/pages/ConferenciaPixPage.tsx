...
import { Loader2, ShieldCheck, RefreshCw, AlertTriangle, Copy, Ban, FileText, Send, Undo2, Trash2, Download } from "lucide-react";
...
export default function ConferenciaPixPage() {
  ...
  const aprovar = async () => {
    ...
    try {
      await chamar("aprovar", { senha, codigo: codigo || null });
      setSenha(""); setCodigo(""); setConferiDivergencia(false);
        ...
        <>
          ...
          {podeAprovar && (
            <Card className="p-4 flex flex-col gap-3">
              ...
              {lote.exige_codigo ? (
                <p className="text-sm text-muted-foreground">
                  {Number(lote.teto_aplicado ?? 0) > 0
                    ? `Este lote passa do teto de ${fmtReais(Number(lote.teto_aplicado))}, então além da sua senha é preciso o código enviado ao aprovador.`
                    : "Todo pagamento precisa do código enviado ao aprovador, além da sua senha."}
                </p>
              ) : (
                <p className="text-sm text-muted-foreground">
                  Abaixo do teto de {fmtReais(Number(lote.teto_aplicado ?? 0))}: basta a sua senha do sistema.
                </p>
              )}
              ...
              <div className="flex gap-2">
                <Button onClick={aprovar} disabled={enviando || (divergentes.length > 0 && !conferiDivergencia)}>
