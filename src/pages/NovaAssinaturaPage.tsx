...
// Soma n meses a uma data ISO, mantendo o dia (limitado ao último dia do mês)
...
export default function NovaAssinaturaPage() {
  ...
  const minAssinanteDesde = isoAddDias(-365);
  ...
  // Próxima cobrança da assinatura (a deste mês pode já ter ocorrido → parte de hoje).
  // Se não houver dia de vencimento, usa "Assinante desde" (ou hoje) como âncora.
  const proximaCobranca = () => {
    const hoje = isoAddDias(0);
    ...
    if (dia >= 1 && dia <= 31) {
      ...
      base = cand < hoje ? addMonthsISO(cand, 1) : cand;
    } else {
      base = form.assinante_desde || hoje;
    }
    // Se a assinatura começa no futuro, avança até a 1ª cobrança dentro da vigência
    while (form.assinante_desde && base < form.assinante_desde) base = addMonthsISO(base, 1);
    return base;
  };
  // 1ª entrada do bônus = "freq cobranças pra frente". freq 1 (ou único) = próxima cobrança;
  // freq 3 = 3ª cobrança = próxima cobrança + 2 meses.
  const calcPrimeiraEntrada = (freqStr: string) => {
    const freq = Number(freqStr) || 0;
    const meses = freq <= 1 ? 0 : freq - 1;
    return addMonthsISO(proximaCobranca(), meses);
  };
  const addBonus = () => setBonus((b) => [...b, { pontos: "", frequencia_meses: "0", repeticoes: "1", primeira_entrada: calcPrimeiraEntrada("0") }]);
  const addBonusDoModelo = (modeloId: string) => {
    ...
    setBonus((b) => [...b, {
      ...
      repeticoes: m.repeticoes?.toString() ?? "1", primeira_entrada: calcPrimeiraEntrada(m.frequencia_meses?.toString() ?? "0"),
    }]);
  };
  ...
  const handleSave = async () => {
    ...
    if (form.assinante_desde && form.assinante_desde < minAssinanteDesde) {
      toast.error("A data 'Assinante desde' não pode ser mais de 365 dias no passado."); return;
    }
    ...
    <div className="space-y-4 max-w-3xl">
      ...
      <Card className="p-4 sm:p-6">
        <div className="grid gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="grid gap-1"><Label>Assinatura Clube (entra no estoque)</Label><Input type="number" min={0} value={form.credito_base} onChange={(e) => set("credito_base", e.target.value)} /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="grid gap-1"><Label>Valor parcela (R$)</Label><NumericInput decimal prefix="R$ " value={Number(form.valor_parcela) || 0} onChange={(n) => set("valor_parcela", String(n))} /></div>
            <div className="grid gap-1"><Label>Dia vencimento</Label><Input type="number" min={1} max={31} value={form.dia_vencimento} onChange={(e) => {
              const d = e.target.value.replace(/\D/g, "").slice(0, 2);
              if (d === "") { set("dia_vencimento", ""); return; }
              const n = Math.min(31, Math.max(1, Number(d)));
              set("dia_vencimento", String(n));
            }} placeholder="1 a 31" /></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="grid gap-1"><Label>Assinante desde</Label><Input type="date" min={minAssinanteDesde} max={maxAssinanteDesde} value={form.assinante_desde} onChange={(e) => set("assinante_desde", e.target.value)} /></div>
            <div className="grid gap-1"><Label>Cartão virtual (4 últimos dígitos)</Label><Input inputMode="numeric" maxLength={4} value={form.cartao_virtual} onChange={(e) => set("cartao_virtual", e.target.value.replace(/\D/g, "").slice(0, 4))} /></div>
          </div>
              ...
              <div className="space-y-2">
                ...
                {bonus.map((b, i) => (
                  <div key={i} className="grid grid-cols-[1fr_1.2fr_0.7fr_1fr_auto] gap-2 items-center">
                    <Input type="number" min={0} value={b.pontos} onChange={(e) => setBonusField(i, "pontos", e.target.value)} />
                    <Select value={b.frequencia_meses} onValueChange={(v) => setBonus((rows) => rows.map((row, idx) => idx === i ? { ...row, frequencia_meses: v, primeira_entrada: calcPrimeiraEntrada(v) } : row))}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
