  ...
  const confirmarPagamento = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .in("id", ids);
      if (error) throw error;
      toast.success(`${contar(ids.length, "pagamento a fornecedor registrado", "pagamentos a fornecedor registrados")}.`);
      setSel(new Set());
      setConfirm(false);
      ...
      <div className="flex items-center justify-between">
        ...
        {!verPagos && (
          <div className="flex items-center gap-2">
            <PagarPorPixButton tipo="fornecedor" ids={[...sel]} />
      ...
      <Dialog open={confirm} onOpenChange={(o) => { if (!o) setConfirm(false); }}>
        <DialogContent className="sm:max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            {contar(sel.size, "emissão selecionada", "emissões selecionadas")}, total <b>{fmtReais(valorSelecionado)}</b>. Informe de qual banco saiu o Pix e a data do pagamento.
          </p>
