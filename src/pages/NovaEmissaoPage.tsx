...
import { vLocalizador, vCodigoLA, vMilhas, vNaoNegativo, vNumPax, vDataVoo } from "@/lib/validacoesEmissao";
...
const emptyForm = {
  ...
  conta_id: "", cliente_id: "", passageiros_qtd: 1,
  cpfs_otimizados: "",
  ...
  ajuste_cupom: "", ajuste_hack_upgrade: false, ajuste_retarifacao: false, ajuste_desconto_promo: false, ajuste_campo_aberto: "",
  cartao_id: "", forma_pagamento: "", codigo_la: "", percentual_cb: 0, origem_venda: "",
  observacao: "", facial: true, nota: "",
};
...
export default function NovaEmissaoPage() {
  ...
  const salvar = async () => {
    ...
    try {
      const criada = await upsert.mutateAsync({
        ...
        percentual_cb: ehLatam ? (form.percentual_cb || null) : null,
        facial: ehLatam && !!form.facial,
        outros_descricao: form.outros_cobrado > 0 ? (form.outros_descricao || null) : null,
        cpfs_otimizados: ehSmiles && form.cpfs_otimizados ? (parseInt(form.cpfs_otimizados) || null) : null,
      ...
      navigate("/", (criada as any)?.id ? { state: { cobrarEmissao: criada } } : undefined);
    } catch (err: any) {
  ...
  const ehSmiles = (form.programa || "").toLowerCase() === "smiles";
  ...
  return (
    <div className="space-y-5 max-w-6xl mx-auto">
      ...
      <Secao titulo="Dados da Emissão">
        ...
        <div className="grid gap-1"><Label>Data Voo Ida *</Label><Input type="date" value={form.data_voo_ida} onChange={(e) => set("data_voo_ida", e.target.value)} className={vDataVoo(form.data_voo_ida, form.data_emissao) ? "border-destructive" : ""} /><CampoErro msg={vDataVoo(form.data_voo_ida, form.data_emissao)} /></div>
        ...
        {/* H */}
        <div className="grid gap-1"><Label>Conta *</Label><SearchSelect value={form.conta_id} onChange={(v) => set("conta_id", v)} options={[...contasFiltradas].sort((a, b) => (a.codigo ?? "").localeCompare(b.codigo ?? "", "pt-BR", { numeric: true })).map((c) => ({ value: c.id, label: `${c.codigo} - ${c.nome}` }))} /></div>
        {/* I (Cliente, maior) + J */}
        <div className="grid gap-1 col-span-2"><Label>Cliente *</Label><SearchSelect value={form.cliente_id} onChange={(v) => set("cliente_id", v)} options={[...(clientes ?? [])].sort((a, b) => (a.codigo ?? "").localeCompare(b.codigo ?? "", "pt-BR", { numeric: true })).map((c) => ({ value: c.id, label: `${c.codigo} - ${c.nome_fantasia}` }))} /></div>
        ...
        {ehLatam && (
          <>
            ...
            <div className="grid gap-1"><Label>% Cashback</Label><NumericInput value={form.percentual_cb} onChange={(n) => set("percentual_cb", n)} decimal placeholder="0,00" /></div>
            <div className="grid gap-1">
              ...
              <label className="flex items-center gap-2 h-10 px-3 rounded-md border cursor-pointer select-none">
                <input type="checkbox" className="h-4 w-4 accent-[hsl(var(--primary))]" checked={!!form.facial} onChange={(e) => setForm((f) => ({ ...f, facial: e.target.checked }))} />
      ...
      <Secao titulo="Valores Cobrados">
        ...
        {form.outros_cobrado > 0 && (
          <div className="grid gap-1 col-span-2 md:col-span-4">
