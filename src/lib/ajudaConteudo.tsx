...
export const AJUDA: SecaoAjuda[] = [
  {
    ...
    passos: [
      ...
      "3) Na lista de Emissões, clique no avião (✈️) para Cobrar: o sistema gera o Pix no banco (Sicoob; se o Sicoob estiver indisponível, usa o Sicredi automaticamente), grava na emissão e envia a mensagem (resumo + QR + copia-e-cola) por WhatsApp para o usuário que emitiu.",
    ...
    regras: [
      ...
      { campo: "Banco emissor do Pix", regra: "O Pix sai pelo Sicoob; se o Sicoob falhar, o sistema tenta o Sicredi na sequência (o banco usado fica registrado na emissão). Independe de qual usuário está emitindo." },
      ...
      { campo: "Forma de cobrança", regra: "Hoje somente Pix. A cobrança por cartão não está disponível." },
      ...
      { campo: "Cancelar Cobrança", regra: "O botão 'Cancelar Cobrança' cancela a cobrança no banco em que foi gerada (Sicoob ou Sicredi) e marca a emissão como CANCELADO. Você recebe a confirmação na tela e por WhatsApp. Cobrança já PAGA não pode ser cancelada." },
      ...
      { campo: "Valores Cobrados x Valores Reais", regra: "Toda emissão (normal ou terceirizada) registra o que foi COBRADO do cliente e o que foi REAL (o custo efetivo). Quando os dois divergem em Qtde Milhas, Taxas, Bagagens, Assentos ou Outros — em valor OU no tipo R$/Milhas —, abre a seção 'Ajustes' e é obrigatório justificar a diferença com ao menos um campo antes de salvar." },
      { campo: "As duas Retarifações", regra: "São dois ajustes diferentes: 'Retarifação' é só a marcação, para quando a tarifa mudou mas a emissão saiu no mesmo programa e na mesma conta. 'Retarifação (Outro Programa)' é para quando o cliente pediu um programa e a emissão saiu por outro (companhia parceira): ela abre 'Programa Emitido' e 'Conta', a cobrança continua mostrando o programa que o cliente pediu e a baixa de milhas vai para a conta e o programa realmente usados." },
      { campo: "Valores em moeda estrangeira", regra:
        "Nas emissões internacionais as taxas (e às vezes bagagens, assentos ou outros) são pagas em dólar, euro, libra ou franco suíço. Nesses casos o seletor do campo ganha a opção 'Moeda', ao lado de R$ e Milhas.\n" +
        "Escolha a moeda, digite o valor e clique no botão da setinha: o sistema busca a cotação do momento, soma o spread cadastrado (Configurações → Moedas, 8% por padrão) e preenche o valor em reais. Esse valor continua editável — se o câmbio da fatura vier diferente, corrija na mão.\n" +
        "Vale nos quatro campos (Taxas, Bagagens, Assentos e Outros) e nos dois blocos (Cobrados e Reais), cada um independente: dá para ter a taxa em euro e a bagagem em reais na mesma emissão.\n" +
        "Cada programa tem suas próprias moedas, marcadas em Configurações → Programas → editar → aba 'Moedas'. O seletor mostra só as moedas daquele programa; sem nenhuma marcada, a opção 'Moeda' nem aparece (é o caso de Latam, Smiles e Azul). Com uma única moeda marcada, ela já vem preenchida e o emissor só digita o valor.\n" +
        "Programas que nunca são pagos em reais (Iberia, Flying Blue) têm a regra 'Aceita valores em reais' desligada: aí o botão R$ some e o campo já abre em moeda estrangeira.\n" +
        "Ficam gravados a moeda, o valor original e a cotação usada, para conferir a conversão depois. O que entra no Preço Total e nos relatórios é sempre o valor em reais." },
      { campo: "Regras por programa", regra:
        "O que cada programa mostra na emissão é configurável em Configurações → Programas → editar o programa → aba 'Regras'. São 13 chaves liga/desliga:\n" +
        "Campos da emissão: CPFs Otimizados, Código LA, % Cashback, Pagar facial?, Aceita valores em reais e Milhas abaixo de 1.000.\n" +
        "Ajustes (só aparecem quando Reais ≠ Cobrados): Cupom (%), Hack Upgrade, Retarifação, Retarifação (Outro Programa), Taxa de Resgate, Desconto Promocional e Campo Aberto.\n" +
        "Outros: Taxa de Queima de CPF (o valor por pax continua em Configurações → Taxa Queima CPF).\n" +
        "'Milhas abaixo de 1.000' não some com campo nenhum — ela só derruba o piso da Qtde Milhas de 1.000 para 100 (hoje ligada no Azul Liminar).\n" +
        "Regra desligada = o campo nem aparece para o emissor, e o valor não é gravado. Programa novo nasce só com o Campo Aberto ligado — ligue o resto no cadastro.\n" +
        "O único campo de ajuste que pede valor é o Cupom (%), em percentual com 2 casas decimais; os demais são caixas de marcar e o Campo Aberto é texto livre." },
    ],
    observacoes: [
      ...
      "A sessão dura 12 horas a partir do login: 5 minutos antes de acabar aparece um aviso na tela, e no fim o app volta para a tela de login. Salve o que estiver preenchendo quando o aviso aparecer — entrar de novo não recupera formulário aberto.",
      "Cada login só funciona em um lugar por vez: quando alguém entra com um usuário, as sessões desse mesmo usuário abertas em outro navegador, computador ou aba anônima caem para a tela de login com o aviso \"Sua sessão foi encerrada porque esta conta foi acessada em outro dispositivo\". Várias abas do mesmo navegador contam como uma sessão só e não se derrubam. Se duas pessoas precisam usar o app ao mesmo tempo, cada uma precisa do seu próprio usuário.",
      "Os Ajustes de emissão servem só para justificar/auditar a diferença entre Valores Reais e Cobrados — não alteram cálculo, estoque nem cobrança.",
    ],
  },
  {
    chave: "emissoes",
    ...
    regras: [
      ...
      { campo: "Cobrança (Pix)", regra: "Ao cobrar, o sistema gera o Pix no Sicoob (ou no Sicredi, se o Sicoob estiver indisponível) e envia a mensagem (resumo + QR + copia-e-cola) por WhatsApp para o usuário que está emitindo. Validade do Pix: 1 ano. A cobrança por cartão não está disponível." },
  ...
  },
  {
    ...
    passos: [
      ...
      "Em 'Valores Cobrados', informe milhas, milheiro, taxas, bagagens, assentos e outros — o Preço Total é calculado sozinho somando tudo. No seletor ao lado de cada campo escolha se o valor é em R$ ou em MILHAS (e, nos programas internacionais, em MOEDA estrangeira).",
    ...
    regras: [
      ...
      { campo: "Código LA", regra: "Aparece quando a regra 'Código LA' está ligada para o programa (Configurações → Programas → aba Regras). Hoje está ligada só na Latam. Quando aparece é obrigatório, deve começar com 'LA' e ter 13 caracteres. Se digitar algo diferente, o campo fica vermelho com o aviso." },
      { campo: "% Cashback / Pagar facial?", regra: "Aparecem quando as regras '% Cashback' e 'Pagar facial?' estão ligadas para o programa (hoje só na Latam). Nos demais programas ficam ocultos e não são gravados." },
      { campo: "Qtde Milhas", regra: "Deve ser 0 ou no mínimo 1.000. Número inteiro. Nos programas com a regra 'Milhas abaixo de 1.000' ligada (Configurações > Programas > Regras), o mínimo cai para 100." },
      { campo: "Nº de Pax", regra: "Obrigatório, no mínimo 1 passageiro." },
      { campo: "CPFs Otimizados", regra: "Aparece ao lado do Nº de Pax quando a regra 'CPFs Otimizados' está ligada para o programa (hoje só na Smiles). Opcional; se preenchido, deve ser entre 1 e o Nº de Pax." },
      ...
      { campo: "Ajustes (Reais ≠ Cobrados)", regra: "Quando os Valores Reais diferem dos Cobrados, abre a seção de Ajustes. Quais campos aparecem depende das regras ligadas para o programa em Configurações → Programas → aba Regras: Cupom (%), Hack Upgrade, Retarifação, Retarifação (Outro Programa), Taxa de Resgate, Desconto Promocional e Campo Aberto. É obrigatório preencher ao menos um." },
      { campo: "Ajustes — o que conta como diferença", regra: "A seção abre quando Cobrado e Real divergem em Qtde Milhas, Taxas, Bagagens, Assentos ou Outros. Vale tanto o VALOR quanto o TIPO: cobrar Taxas em R$ e registrar o real em Milhas já conta como diferença, mesmo com o mesmo número. Preço Milheiro e Preço Total NÃO entram nessa comparação." },
      { campo: "Ajustes — campos por programa", regra:
        "Cupom (%) é o único campo de valor (percentual com 2 casas decimais, ex.: 5,00). Hack Upgrade, Retarifação, Taxa de Resgate e Desconto Promocional são caixas de marcar (apesar do nome, a Taxa de Resgate não pede valor). Campo Aberto é texto livre.\n" +
        "Configuração atual: Smiles — Cupom, Hack Upgrade, Retarifação, Desconto Promocional e Campo Aberto. Azul Viagens — Cupom, Retarifação e Campo Aberto. Latam — Retarifação, Desconto Promocional e Campo Aberto. Azul Liminar — Retarifação, Taxa de Resgate e Campo Aberto. Interline e Iberia — Retarifação e Campo Aberto. Demais programas — apenas o Campo Aberto.\n" +
        "Isso não é fixo no sistema: mude em Configurações → Programas → editar o programa → aba Regras." },
      { campo: "Ajustes — quem configura", regra: "Os campos vêm das regras marcadas no cadastro do programa (Configurações → Programas → aba Regras), não do nome do programa. Se um campo esperado não aparece, é porque a regra está desligada para aquele programa — não é erro de digitação do nome." },
      { campo: "Ajustes — preenchimento mínimo", regra: "Com valores diferentes, o salvamento é bloqueado até preencher ao menos um ajuste. O Cupom só conta se for MAIOR QUE ZERO (0,00 não vale) e o Campo Aberto só conta com texto digitado; as caixas de marcar contam quando marcadas." },
      { campo: "As duas Retarifações", regra:
        "'Retarifação' é só a marcação: a tarifa mudou, mas a emissão saiu no mesmo programa e na mesma conta do cabeçalho. Não abre campo nenhum.\n" +
        "'Retarifação (Outro Programa)' é a que muda o jogo — use quando a emissão saiu em outra companhia. As duas são liberadas separadamente em Configurações → Programas → aba Regras, e podem estar ligadas ao mesmo tempo." },
      { campo: "Retarifação (Outro Programa) — Programa Emitido e Conta", regra:
        "Use quando o cliente pediu um programa mas a emissão saiu de fato em outro (companhia parceira, com custo melhor).\n" +
        "Ao marcar, abrem dois campos, os DOIS obrigatórios: 'Programa Emitido' (onde a emissão realmente saiu) e 'Conta' (só lista contas que participam desse programa). Trocar o Programa Emitido limpa a Conta.\n" +
        "A Conta lá de cima, no cabeçalho, deixa de ser pedida — a conta gravada na emissão passa a ser a da Retarifação, para não ficar conta que não foi usada.\n" +
        "O Programa do cabeçalho NÃO muda: é o que o cliente pediu e é o que aparece na cobrança, no WhatsApp e nos relatórios.\n" +
        "A baixa de estoque usa a Conta + o Programa Emitido. Se esse programa não tiver Programa de Estoque vinculado, o sistema bloqueia o salvamento (senão a emissão não baixaria de lugar nenhum).\n" +
        "Depois de escolher o Programa Emitido, as REGRAS da emissão passam a ser as dele: os campos liberados (Código LA, % Cashback, Pagar facial?, CPFs Otimizados), os demais Ajustes e as moedas aceitas seguem o programa onde a emissão saiu, não o do cabeçalho. A única exceção é a própria 'Retarifação (Outro Programa)', que continua sendo liberada pelo programa do cabeçalho.\n" +
        "Desmarcar, ou igualar os Valores Reais aos Cobrados, descarta o Programa Emitido e tudo volta para o programa do cabeçalho." },
      ...
      { campo: "Facial Realizada?", regra: "Aparece quando a regra 'Pagar facial?' está ligada para o programa (hoje só na Latam). Já vem marcado como Sim por padrão (caso mais comum) — desmarque se a emissão não for pagamento facial. O valor e a data do facial NÃO são informados aqui — o pagamento é lançado depois na tela 'Pagamento Facial'." },
    ],
    observacoes: [
      ...
      "A aba 'Cobrança' (somente leitura) mostra a forma, o Banco Emissor (Sicoob ou Sicredi), o status, o TXID e o copia-e-cola do Pix, além de data/valor recebido. A Observação registra automaticamente por qual banco e em que data/hora (Brasília) o Pix foi emitido. Tudo preenchido pelo sistema.",
      ...
      "Os Ajustes são apenas justificativa/auditoria da diferença: não alteram o Preço Total, nem o estoque de milhas, nem a cobrança do cliente.",
      "Os Ajustes só são gravados se, no momento de salvar, os valores continuarem diferentes E o campo estiver liberado para o programa. Se você preencher um ajuste e depois igualar os valores (ou trocar o programa), o que foi preenchido é descartado ao salvar.",
    ],
  },
  {
    ...
    regras: [
      ...
      { campo: "Reenviar", regra: "Reenvia a mensagem completa + QR Code do MESMO Pix por WhatsApp (para o usuário que emitiu), lendo só o banco de dados — não chama a API do banco nem gera novo Pix. Funciona igual para cobranças do Sicoob e do Sicredi, e para Emissões diretas ou Terceirizadas." },
  ...
  },
  {
    ...
    regras: [
      ...
      { campo: "CEP (busca de endereço)", regra: "Digite o CEP completo (8 dígitos) e clique na lupa ao lado do campo — o sistema busca o endereço online e preenche Logradouro, Bairro, Município e UF. Só resta completar o Número." },
      { campo: "E-mail", regra: "Obrigatório — a NFS-e é enviada por e-mail." },
      { campo: "Telefone", regra: "Obrigatório. Escolha o país (padrão Brasil +55) e digite o número — a máscara é aplicada sozinha. No Brasil, ao começar com 7, 8 ou 9 usa o formato de celular (DD) 9XXXX-XXXX; senão, fixo (DD) XXXX-XXXX. É salvo com o código do país (ex.: +55 (54) 99999-9999)." },
  ...
  },
  {
    ...
    passos: [
      ...
      "Use 'Exportar Excel' para baixar os dados filtrados em .xlsx (abre direto no Excel).",
  ...
  },
  {
    ...
    regras: [
      ...
      { campo: "Cobrança ao cliente", regra: "Igual às Emissões normais: Cobrar/Reprocessar/Cancelar via Pix (Sicoob/Sicredi), mesma validade de 1 ano e mesmas regras de edição/exclusão por dono." },
  ...
  },
  {
    ...
    regras: [
      ...
      { campo: "CPFs Otimizados", regra: "Não existe nesta tela, mesmo com a regra ligada para o programa." },
      { campo: "% Cashback / Pagar facial?", regra: "Não existem nesta tela, mesmo com as regras ligadas. O Código LA continua aparecendo (e obrigatório) quando a regra 'Código LA' está ligada para o programa." },
      ...
      { campo: "Ajustes (Reais ≠ Cobrados)", regra: "Funcionam exatamente como na Nova Emissão normal. A seção abre quando Cobrado e Real divergem em Qtde Milhas, Taxas, Bagagens, Assentos ou Outros — valor OU tipo (R$/Milhas). Preço Milheiro, Custo Milheiro, Preço Total e Custo Total não entram na comparação." },
      { campo: "Ajustes — campos por programa", regra:
        "Iguais aos da Nova Emissão normal: quais campos aparecem vem das regras ligadas no cadastro do programa (Configurações → Programas → aba Regras).\n" +
        "Cupom (%) é o único campo de valor (percentual com 2 casas decimais). Hack Upgrade, Retarifação, Taxa de Resgate e Desconto Promocional são caixas de marcar. Campo Aberto é texto livre." },
      { campo: "Ajustes — preenchimento mínimo", regra: "Com valores diferentes, é obrigatório preencher ao menos um ajuste para salvar. O Cupom só conta se for maior que zero e o Campo Aberto só conta com texto digitado." },
      { campo: "Retarifação (Outro Programa) — Programa Emitido", regra: "São dois ajustes: 'Retarifação' é só a marcação (mesmo programa), e 'Retarifação (Outro Programa)' abre o 'Programa Emitido' (obrigatório), para registrar em que programa a emissão realmente saiu. Aqui é só registro: emissão terceirizada não consome milhas próprias, então não há baixa de estoque nem Conta a informar (diferente da Nova Emissão normal). Como na emissão normal, depois de escolhido o Programa Emitido as regras e as moedas passam a ser as dele." },
      { campo: "Valores em moeda estrangeira", regra: "Funciona igual à Nova Emissão normal: nos programas com moedas marcadas (Configurações → Programas → aba Moedas), Taxas, Bagagens, Assentos e Outros ganham a opção 'Moeda' no seletor, em Cobrados e em Reais. Informe a moeda e o valor, busque a cotação pelo botão da setinha e o sistema converte para reais somando o spread de Configurações → Moedas. O valor convertido continua editável e é ele que entra no Preço Total e no Custo Total." },
    ],
    observacoes: [
      "Os Ajustes são apenas justificativa/auditoria da diferença: não alteram o Preço Total, o Custo Total, o estoque de milhas nem a cobrança do cliente.",
      "Os Ajustes só são gravados se, ao salvar, os valores continuarem diferentes E a regra do campo estiver ligada para o programa — caso contrário são descartados.",
    ],
  },
  {
    chave: "fornecedores",
  ...
  {
    chave: "conferencia_pix",
    telaChave: "conferencia_pix",
    titulo: "Conferência Pix",
    resumo:
      "Onde o lote de pagamentos é conferido e aprovado antes de qualquer dinheiro sair. Os lotes chegam aqui pelo botão 'Pagar por Pix' das telas de Pagamentos (Faciais, Fornecedores e Reembolsos). Nenhum Pix é enviado sem passar por esta tela.",
    passos: [
      "Nas telas de Pagamentos, marque as linhas e clique em 'Pagar por Pix'. O sistema monta o lote e traz você para cá.",
      "O sistema pergunta ao Banco Central o nome do titular de cada chave Pix (uma consulta por segundo). Espere a coluna 'Titular (Banco Central)' preencher.",
      "Confira linha a linha: o nome que o Banco Central devolveu tem que bater com o do cadastro. Linhas com nome diferente ficam destacadas em vermelho.",
      "Digite a sua senha do sistema, peça o código do aprovador (vai por e-mail para o admin) e os 6 dígitos que estão no aplicativo Google Authenticator do aprovador. Todo pagamento precisa das três, sempre.",
      "Clique em 'Aprovar e pagar'. A partir daí os Pix são enviados e cada linha vai virando 'Pago' ou 'Rejeitado'.",
      "Mudou de ideia antes de aprovar? Clique em 'Reverter': o lote é desfeito, os itens voltam para a lista de onde vieram e você paga do jeito de sempre.",
      "Em cada linha paga, o botão 'Comprovante' copia o comprovante pronto para o operador colar onde precisar.",
    ],
    regras: [
      { campo: "De onde vem a chave", regra: "Só do cadastro, nunca adivinhada. Facial: Chave Pix (envio) do cadastro da Conta. Fornecedor: Chave Pix do cadastro do Fornecedor. Reembolso: devolução pelo Pix de origem quando o recebimento foi pelo Sicoob nos últimos 90 dias; senão, Chave Pix do cadastro do Cliente." },
      { campo: "O que o favorecido vê", regra: "Cada Pix leva uma descrição curta, que aparece no extrato de quem recebe. Facial: o código da conta (ex.: P120). Fornecedor: o localizador (ex.: ABCDEF). Reembolso: \"Reembolso\" mais o localizador (ex.: Reembolso ABCDEF)." },
      { campo: "Quantos Pix saem", regra: "Facial: UM Pix por conta. Várias emissões da mesma conta viram um pagamento só, pelo valor somado — a linha na tela mostra as emissões juntas e o aviso 'X emissões num Pix só'. Fornecedores e reembolsos: um Pix por localizador, para o favorecido casar cada crédito com a emissão dele." },
      { campo: "Sem chave, não monta", regra: "Se QUALQUER selecionado estiver sem chave Pix no cadastro, o lote nem chega a ser criado: aparece o nome de quem falta e onde cadastrar. Ou todos têm chave, ou você completa o cadastro e tenta de novo — não existe pagar metade." },
      { campo: "Descartar", regra: "Lote encerrado que não pagou nada (o banco recusou tudo) some da lista com o botão Descartar — não serve para nada e só atrapalha. Lote que chegou a pagar alguma coisa nunca é apagado: vira histórico financeiro." },
      { campo: "Titular (Banco Central)", regra: "Não vem do nosso cadastro: é o nome que o Banco Central devolve para aquela chave. É a trava principal contra chave trocada. Enquanto faltar consultar alguma linha, o lote não pode ser aprovado." },
      { campo: "Nome divergente", regra: "Linha destacada em vermelho quando o titular não bate com o cadastro. Não bloqueia sozinha — quem aprova assume a conferência." },
      { campo: "Não serão pagos", regra: "Linhas sem chave Pix no cadastro entram como bloqueadas, ficam fora do total e seguem pelo caminho manual de sempre. Cadastre a chave e monte o lote de novo para incluí-las." },
      { campo: "Código do aprovador", regra: "Todo pagamento, de qualquer valor, precisa da senha de quem está aprovando MAIS um código de 6 dígitos enviado por e-mail ao admin. O código vale 10 minutos e só serve para aquele lote exato — se a lista mudar, o código perde a validade." },
      { campo: "Google Authenticator", regra: "Terceira validação, obrigatória SEMPRE: 6 dígitos do aplicativo no celular do aprovador, que trocam a cada 30 segundos. Ao contrário do código por e-mail, ele não trafega por lugar nenhum — continua valendo mesmo se a caixa de e-mail do aprovador for invadida. Cada código serve uma vez só. Só admin cadastra o aplicativo (lendo o QR Code nesta tela), porque um operador que pudesse cadastrar apontaria o app para o próprio celular. Enquanto o aplicativo não estiver cadastrado, NENHUM pagamento é liberado — nem com senha e código do e-mail certos." },
      { campo: "Trocar de aparelho", regra: "O admin gera um QR novo e lê no aparelho novo. O aparelho antigo continua valendo até a troca ser confirmada com um código do novo — troca abandonada no meio não derruba a validação. Celular perdido: o admin refaz o cadastro aqui; até refazer, os pagamentos ficam parados, que é o preço de a validação ser obrigatória." },
      { campo: "Devolução parcial", regra: "No reembolso pelo Pix de origem, o valor devolvido é o do reembolso, que pode ser menor que o recebido. O banco aceita devolver parte e o restante continua com a empresa." },
      { campo: "Reverter", regra: "Desiste do pagamento por Pix, apaga o lote e volta para a tela de origem. Aparece enquanto o lote não foi aprovado — até aí nenhum dinheiro saiu, o sistema só perguntou ao Banco Central quem é o dono de cada chave. Os itens voltam para a lista normal e podem ser pagos do jeito de sempre, ou selecionados de novo depois." },
      { campo: "Comprovante", regra: "Mostrado na tela com opção de copiar. O envio é sempre para o operador — o sistema não manda nada direto para fornecedor ou cliente." },
      { campo: "Acesso", regra: "Tela controlada por permissão. Quem não tem acesso a ela também não vê o botão 'Pagar por Pix' nas telas de Pagamentos." },
    ],
    observacoes: [
      "O mesmo item não entra em dois lotes vivos ao mesmo tempo — o banco recusa a duplicidade.",
      "Reenviar uma aprovação não paga duas vezes: cada linha tem um identificador único de transação.",
      "Aprovar não dá baixa sozinho: a baixa na tela de origem acontece quando o banco confirma o pagamento.",
    ],
  },
  {
  ...
  },
  {
    ...
    passos: [
      ...
      "Use 'Exportar Excel' para baixar os dados filtrados em .xlsx.",
    ],
    regras: [
  ...
  {
    chave: "dashboard_faturamento",
    telaChave: "dashboard_faturamento",
    titulo: "Dashboard de Faturamento",
    resumo:
      "Visão gerencial do faturamento: indicadores do período, faturamento por programa e por mês, faixas de precificação, gráficos, ranking de agências e calendários diários.",
    passos: [
      "Escolha a origem (próprias, terceirizadas ou as duas) e refine por programa, emissor, cliente, status ou período.",
      "Os botões de mês selecionam o mês inteiro de uma vez. Todos os blocos da tela reagem aos filtros.",
      "Os cards do topo mostram o faturamento de cada mês e o total do ano; abaixo vêm os 15 indicadores do período filtrado.",
      "As tabelas 'Programa por mês' trazem o valor e, embaixo, a participação do programa naquele mês.",
      "No Detalhamento, clique no cabeçalho 'Data' para inverter a ordem; a tabela pagina de 150 em 150. 'Exportar Excel' baixa todos os registros do filtro de uma vez.",
    ],
    regras: [
      { campo: "Acesso", regra: "Tela gerencial: liberada para administradores. Operador que receber acesso enxerga somente as próprias emissões, como nas demais telas." },
      { campo: "O que conta como emissão", regra: "Só as operações com Nome Operação = 'Emissão'. Remarcação, Queima CPFs, Assento, Bagagem, Taxa Reembolso e Upgrade entram no faturamento, mas não contam como emissão nem somam PAX." },
      { campo: "Faturamento", regra: "Soma o Preço Total de TODAS as operações do filtro, inclusive as pós-vendas e as canceladas (o valor cancelado também aparece no card 'Cancelado')." },
      { campo: "Venda de Milhas", regra: "Qtde Milhas × Preço do Milheiro ÷ 1.000 — só as milhas, sem taxas, bagagens, assentos ou outros. 'Taxas + Outros' é o faturamento menos esse valor." },
      { campo: "Preço Médio do Milheiro", regra: "Sempre média PONDERADA pelas milhas, nunca média simples dos preços — emissões grandes pesam mais." },
      { campo: "Data considerada", regra: "É a Data da Emissão (competência), não a data do recebimento." },
      { campo: "Taxas em milhas ou em moeda", regra: "Taxas, bagagens, assentos e outros lançados em milhas são convertidos pelo preço do milheiro; em moeda estrangeira já entram convertidos em reais." },
      { campo: "Faixas de precificação", regra: "Classificam cada emissão por Qtde Milhas ÷ PAX, só para Smiles e Latam, e só em operações do tipo Emissão com PAX maior que zero. Por isso o total das faixas é menor que o faturamento do programa." },
      { campo: "Semanas", regra: "Semana ISO, de segunda a domingo — nos gráficos semanais e nos calendários." },
    ],
    observacoes: [
      "Reproduz o dashboard que o cliente montou na planilha, agora lendo direto do banco do sistema.",
      "Emissores que só fazem pós-venda aparecem com zero emissões — está correto, não é falha.",
    ],
  },
  {
  ...
  },
  {
    ...
    titulo: "Promoções",
    resumo:
      "Histórico das campanhas de compra e de transferência de milhas: nome, bancos participantes, programas, vigência e regulamento. Mesmo conceito dos Modelos de Bônus — cadastra uma vez e depois vincula ao lançamento.",
    passos: [
      "Em Estoque → Promoções, clique em 'Nova Promoção'.",
      "Escolha o Tipo: Compra (pede um Programa) ou Transferência (pede Programa origem e destino).",
      "Deixe 'Bancos participantes' em Todos ou desligue a chave e marque os bancos da campanha.",
      "Na promoção de transferência, informe o % de bônus e, se houver, o bônus máximo (teto de milhas bonificadas por conta).",
      "Informe a vigência (data e hora de início e de fim), o link do regulamento e as demais informações. Salve.",
      "Na Nova Transferência (ou Nova Compra), escolha a promoção no campo 'Promoção' — o % de bônus vem preenchido e pode ser editado.",
    ],
    regras: [
      { campo: "Tipo", regra: "Define quais programas a promoção pede. Ao trocar o tipo, os programas do outro tipo não são gravados." },
      { campo: "Bancos participantes", regra: "A lista vem dos Programas de Estoque cadastrados com Tipo = Banco (Configurações › Programas Estoque)." },
      { campo: "% Bônus", regra: "Só na promoção de transferência. É puxado automaticamente ao escolher a promoção no lançamento e continua editável lá." },
      { campo: "Bônus máximo", regra: "Teto de MILHAS BONIFICADAS por conta, somando todas as transferências da promoção. Ex.: teto de 30.000 com bônus de 30% — transferir 100.000 credita 30.000 de bônus (130.000 no total); transferir 110.000 credita os mesmos 30.000 (140.000), porque o excedente entra sem bônus. Deixe 0 para não ter teto." },
      { campo: "Programas", regra: "Deixar em 'Todos' significa que a promoção vale para qualquer programa." },
      { campo: "Vigência", regra: "Opcional. Sem vigência, a promoção é considerada sempre vigente; dentro do período ela aparece com o selo 'vigente' na lista." },
      { campo: "Ativa", regra: "Promoção inativa continua no histórico, mas não é oferecida nos lançamentos." },
      { campo: "Excluir", regra: "Só admin/super admin. Não é possível excluir promoção já vinculada a uma compra ou transferência." },
    ],
    observacoes: [
      "Os bancos vêm do cadastro em Configurações › Bancos.",
    ],
  },
  {
    chave: "estoque",
    ...
    passos: [
      "Em Clubes → Conferência Clubes, veja os blocos: Atrasadas de meses anteriores (em destaque) e depois um bloco por mês (mês atual, mês seguinte, e assim por diante).",
      "Confira cada crédito previsto e clique em 'Confirmar'.",
      "No pop-up, confira os dados e confirme. Na assinatura, o Plano, o Valor e o cartão já vêm preenchidos com o que está cadastrado na assinatura; no bônus aparecem só Data, Hora e Quantidade de milhas. O Motivo é opcional.",
      "Opcional: use os filtros de conta e programa — eles só reduzem a lista; os blocos e as colunas continuam os mesmos.",
    ],
    regras: [
      ...
      { campo: "Blocos por mês", regra: "Cada mês tem seu próprio bloco (Agosto, Setembro, Outubro...), com a quantidade de conferências ao lado do título." },
      { campo: "Assinatura retroativa", regra: "Se a assinatura for lançada com data retroativa, o sistema também cria as conferências dos meses já vencidos (assinatura e bônus), a partir do 'assinante desde'." },
      { campo: "Confirmar", regra: "Abre um pop-up com os campos editáveis. Assinatura: Data, Hora, Plano, Qtde de milhas, Valor e cartão (código e final) já preenchidos pela assinatura. Bônus: só Data, Hora e Qtde de milhas — bônus não tem plano, valor nem cartão. O Motivo é opcional. Só ao confirmar o crédito entra no estoque." },
