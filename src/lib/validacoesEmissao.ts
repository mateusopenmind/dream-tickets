...
// `exigido` vem da regra "Código LA" do programa (Configurações > Programas > Regras).
export function vCodigoLA(v: string, exigido: boolean): string | null {
  if (exigido) {
    if (!v) return "Obrigatório para este programa.";
...
// `minimo` vem da regra "Milhas abaixo de 1.000" do programa (Configurações > Programas > Regras):
// 1.000 no padrão, 100 quando a regra está ligada.
export function vMilhas(n: number, minimo: number = 1000): string | null {
  if (n == null) return null;
  if (n !== 0 && n < minimo) return `Deve ser 0 ou no mínimo ${minimo.toLocaleString("pt-BR")}.`;
  return null;
}
...
export function vNumPax(n: number): string | null {
  // Campo vazio (novo registro) não mostra erro — a validação do salvar cobra o preenchimento.
  if (n == null || (n as unknown as string) === "") return null;
