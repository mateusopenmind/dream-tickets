...
// Operações usadas no lançamento de Perdas de estoque (cadastro em Configurações)
export function useOperacoesPerdas() {
  return useQuery({ queryKey: ["operacoes_perdas"], queryFn: async () => {
    const { data, error } = await (supabase as any).from("operacoes_perdas").select("*").order("nome");
    if (error) throw error;
    return (data ?? []) as { id: string; nome: string }[];
  }});
}
// Moedas estrangeiras aceitas nas emissões (cadastro em Configurações > Moedas).
// `spread_percentual` é o que se soma à cotação de mercado (default 8%).
export type MoedaCad = { id: string; codigo: string; nome: string; spread_percentual: number; ativo: boolean };
export function useMoedas() {
  return useQuery({ queryKey: ["moedas"], queryFn: async () => {
    const { data, error } = await (supabase as any).from("moedas").select("*").order("codigo");
    if (error) throw error;
    return (data ?? []) as MoedaCad[];
  }});
}
// Só as ativas — é o que aparece no seletor da emissão.
export function useMoedasAtivas() {
  const q = useMoedas();
  return { ...q, data: (q.data ?? []).filter((m) => m.ativo !== false) };
}

// Quais moedas cada programa aceita (Configurações > Programas > aba Moedas).
// Retorna { [programa_id]: ["USD", "EUR"] }. Programa fora do mapa não aceita
// moeda estrangeira — é o que libera (ou não) a opção "Moeda" na emissão.
export function useProgramaMoedas() {
  return useQuery({ queryKey: ["programa_moedas"], queryFn: async () => {
    const { data, error } = await (supabase as any)
      .from("programa_moedas")
      .select("programa_id, moedas(codigo, ativo)");
    if (error) throw error;
    const mapa: Record<string, string[]> = {};
    for (const r of (data ?? []) as any[]) {
      if (!r.moedas || r.moedas.ativo === false) continue;
      (mapa[r.programa_id] ??= []).push(r.moedas.codigo);
    }
...
/**
 * Como o programa escolhido se comporta nos campos de valor da emissão.
 * `moedas` vazio = sem moeda estrangeira; `aceitaReais` false = Iberia e afins,
 * que nunca são pagos em reais.
 */
export function useMoedasDoPrograma(nomePrograma: string) {
  const { data: programas, isLoading: carregandoProgramas } = useProgramas();
  const { data: mapa, isLoading: carregandoMoedas } = useProgramaMoedas();
  const p = (programas ?? []).find((x: any) => x?.nome === nomePrograma) as any;
  const moedas = (p && mapa?.[p.id]) || [];
  return {
    moedas,
    // Sem moeda cadastrada não dá para tirar o R$ do emissor, senão ele fica sem opção.
    aceitaReais: moedas.length === 0 || (p?.regra_aceita_reais ?? true) !== false,
    // Enquanto isso é true não dá para concluir nada sobre o programa — quem consome
    // precisa esperar, ou vai "corrigir" um campo em moeda achando que é inválido.
    carregando: carregandoProgramas || carregandoMoedas,
  };
}

// Operações usadas no lançamento de Compras de estoque (cadastro em Configurações)
export function useOperacoesCompras() {
  return useQuery({ queryKey: ["operacoes_compras"], queryFn: async () => {
    const { data, error } = await (supabase as any).from("operacoes_compras").select("*").order("nome");
    if (error) throw error;
    return (data ?? []) as { id: string; nome: string }[];
  }});
}
  ...
  return useMutation({
    mutationFn: async (c: ClienteInsert & { id?: string }) => {
      const payload = {
        ...
        // Chave Pix do cliente: usada para devolver reembolso quando nao der para
        // devolver pelo Pix de origem. Sem ela o item entra bloqueado na Conferencia.
  ...
  return useMutation({
    mutationFn: async (c: ContaInsert & { id?: string; numero_smiles?: string }) => {
      const payload = {
        ...
        fone: c.fone ?? null, email: c.email ?? null, tipo: c.tipo ?? null, pix_envio: c.pix_envio ?? null, pix_titular: c.pix_titular ?? null,
...
export interface ReembInfoEmissao { tipo: string; trecho: string | null; pax_qtd: number | null; pax_total: number | null; }
// Uma emissão pode ter VÁRIOS reembolsos (parcial + 1 trecho ida/volta + taxas) — devolve todos,
// por ordem de criação, ignorando os com cobrança cancelada.
export function useReembolsosPorEmissao(tabelaOrigem: "emissoes" | "emissoes_terceirizadas" = "emissoes") {
  return useQuery({ queryKey: ["reembolsos_por_emissao", tabelaOrigem], queryFn: async () => {
    const map: Record<string, ReembInfoEmissao[]> = {};
    const { data, error } = await supabase
      .from("reembolsos")
      .select("emissao_id, tipo, trecho, pax_qtd, pax_total, status_pix, created_at")
      .eq("tabela_origem", tabelaOrigem)
      .order("created_at", { ascending: true })
      .limit(10000);
    if (error) throw error;
    for (const r of data ?? []) {
      if (!r.emissao_id || r.status_pix === "CANCELADO") continue;
      (map[r.emissao_id] ??= []).push({ tipo: r.tipo, trecho: (r as any).trecho ?? null, pax_qtd: r.pax_qtd, pax_total: r.pax_total });
    }
    return map;
  }});
}
    ...
    queryFn: async (): Promise<CobrancaAberta[]> => {
      ...
      const [normais, terceirizadas, reembs] = await Promise.all([
        ...
        supabase.from("reembolsos")
          .select("id, reembolso_id, cliente_id, created_at, localizador, programa, tipo, pax_qtd, pax_total, preco_total, clientes(codigo, nome_fantasia)")
      ...
      // Reembolsos a cobrar do cliente (Pix em aberto) — Operação = "Reembolso Total/Parcial (X de Y Pax)/Taxas".
      const montarReemb = (rows: any[]): CobrancaAberta[] =>
        (rows ?? []).map((r) => ({
          ...
          localizador: r.localizador ?? "",
          programa: r.programa ?? "",
          nome_operacao: "Reembolso " + rotuloReembolso(r.tipo, r.pax_qtd, r.pax_total),
          data_voo_ida: null,
          preco_total: Number(r.preco_total) || 0,
        }));
...
export interface SaldoPaxReembolso { total_pax: number; tem_total: boolean; ida_usada: number; volta_usada: number; taxas_usada: number; }
// Saldo de pax por trecho já reembolsado da emissão (cross-user). p_excluir ignora o próprio reembolso (edição).
export async function getSaldoPaxReembolso(emissaoId: string, tabela: string, excluir?: string | null) {
  const { data, error } = await (supabase.rpc as any)("saldo_pax_reembolso", {
    p_emissao_id: emissaoId, p_tabela: tabela, p_excluir: excluir ?? null,
  });
  if (error) throw error;
  return (data ?? null) as SaldoPaxReembolso | null;
}
