...
// Primeiro vencimento a partir de uma data (usado quando a assinatura é lançada com data
// retroativa: as conferências dos meses já vencidos também precisam ser geradas).
function primeiroVenctoDesde(desdeISO: string, dia: number): string {
  const [y, m] = desdeISO.split("-").map(Number);
  const lastDay = new Date(y, m, 0).getDate();
  const cand = `${y}-${String(m).padStart(2, "0")}-${String(Math.min(dia, lastDay)).padStart(2, "0")}`;
  return cand < desdeISO ? addMonthsISO(cand, 1) : cand;
}
...
export function useSalvarAssinatura() {
  ...
  return useMutation({
    mutationFn: async (a: AssinaturaInput) => {
      ...
      // Movimentos já confirmados desta assinatura — não geramos previsão repetida para o
      // mesmo mês (a confirmação permite editar a data, por isso a chave é o mês).
      const { data: jaConfirmados } = await sb.from("estoque_movimentos")
        .select("data, tipo, origem")
        .eq("assinatura_id", assinaturaId).eq("confirmado", true);
      // O id do bônus é recriado a cada salvamento, por isso a identificação usa o nome (origem).
      const chaveMes = (tipo: string, data: string, origem?: string | null) =>
        `${tipo}|${(data ?? "").slice(0, 7)}|${tipo === "bonus" ? (origem ?? "") : ""}`;
      const confirmadosSet = new Set(
        (jaConfirmados ?? []).map((m: any) => chaveMes(m.tipo, m.data, m.origem))
      );

      const movimentos: any[] = [];
      // Fim da assinatura: 31/12 do ano atual + 4 (a assinatura projeta os créditos até essa data).
      const fimProjecao = `${new Date().getFullYear() + 4}-12-31`;
      // Crédito base recorrente: cobre os meses já vencidos (quando a assinatura é lançada com
      // data retroativa, a partir de "assinante desde") e vai até o fim da assinatura.
      if ((payload.credito_base || 0) > 0 && payload.dia_vencimento) {
        const passo = payload.periodicidade_meses || 1;
        const inicio = payload.assinante_desde
          ? primeiroVenctoDesde(payload.assinante_desde, payload.dia_vencimento)
          : proximaDataVencto(payload.dia_vencimento);
        const limite = fimProjecao;
        for (let k = 0; k < 240; k++) {
          const data = addMonthsISO(inicio, k * passo);
          if (data > limite) break;
          if (confirmadosSet.has(chaveMes("assinatura", data))) continue;
          movimentos.push({
            conta_id: a.conta_id, programa_id: programaId, programa_estoque_id: programaEstoqueId,
            data, tipo: "assinatura", referencia: data,
      ...
      for (const b of bonusSalvos) {
        ...
        const origemBonus = b.descricao || (freq === 0 ? "Bônus único" : freq === 1 ? "Bônus mensal" : `Bônus a cada ${freq} meses`);
        for (let k = 0; k < reps; k++) {
          const dataBonus = addMonthsISO(b.primeira_entrada, k * freq);
          if (dataBonus > fimProjecao) break;
          if (confirmadosSet.has(chaveMes("bonus", dataBonus, origemBonus))) continue;
          movimentos.push({
            conta_id: a.conta_id, programa_id: programaId, programa_estoque_id: programaEstoqueId,
            data: dataBonus, tipo: "bonus", referencia: dataBonus,
            pontos: Math.round(b.pontos || 0),
            origem: origemBonus,
...
export function useDesconfirmarMovimento() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      // Restaura a data prevista (referencia) ao devolver — não deixa a data confirmada (hoje).
      const { data: row } = await sb.from("estoque_movimentos").select("referencia").eq("id", id).single();
      const upd: any = { confirmado: false, previsto: true, confirmado_em: null };
      if ((row as any)?.referencia) upd.data = (row as any).referencia;
      const { error } = await sb.from("estoque_movimentos").update(upd).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["estoque-movimentos"] }),
  });
...
export interface CompraInput {
  ...
  meio_pagamento?: string | null;  // "cartao" | "pix"
  banco_id?: string | null;        // banco do Pix (cadastro Configurações › Bancos)
  descricao?: string | null;
}
...
export function useEstoqueCompras() {
  return useQuery({ queryKey: ["estoque-compras"], queryFn: async () => {
    const { data, error } = await sb
      ...
      .select("*, contas(codigo, nome), programas_estoque(nome), bancos(nome)")
      .order("data", { ascending: false })
      .limit(10000);
    if (error) throw error;
    return (data ?? []) as any[];
  ...
  return useMutation({
    mutationFn: async (c: CompraInput) => {
      ...
      const payload = {
        ...
        promocao_id: c.promocao_id || null,
        meio_pagamento: c.meio_pagamento ?? "cartao",
...
export interface TransferenciaInput {
  ...
  carrinho?: number;         // fração: 0.20 = 20% da qtde transferida
  promocao_id?: string | null;
  bonus_milhas?: number | null;  // milhas de bônus já com o teto da promoção aplicado
  fator_conversao?: number | null; // paridade origem→destino (pontos da origem por 1 do destino)
  meio_pagamento?: string | null;  // "cartao" | "pix"
  banco_id?: string | null;        // banco do Pix (cadastro Configurações › Bancos)
  custo_total: number | null;
  num_parcelas: number | null;
...
// Milhas do carrinho: sempre um % da quantidade transferida.
export function qtdeCarrinho(transferida: number, carrinho: number): number {
  return Math.round((Math.round(Number(transferida) || 0)) * (Number(carrinho) || 0));
}

// Quantidade que chega no destino: transferida + carrinho, tudo com bônus.
export function qtdeRecebida(transferida: number, bonus: number, carrinho: number = 0): number {
  const t = Math.round(Number(transferida) || 0);
  const c = qtdeCarrinho(t, carrinho);
  return Math.round((t + c) * (1 + (Number(bonus) || 0)));
}
...
export function useEstoqueTransferencias() {
  return useQuery({ queryKey: ["estoque-transferencias"], queryFn: async () => {
    const { data, error } = await sb
      ...
      .select(
        "*, remetente:contas!conta_remetente_id(codigo, nome), recebedora:contas!conta_recebedora_id(codigo, nome), prog_remetente:programas_estoque!programa_estoque_remetente_id(nome), prog_recebedor:programas_estoque!programa_estoque_recebedor_id(nome), bancos(nome)"
      )
      .order("data", { ascending: false })
      .limit(10000);
    if (error) throw error;
...
export function useSalvarTransferencia() {
  ...
  return useMutation({
    mutationFn: async (t: TransferenciaInput) => {
      ...
      const carrinhoPct = Number(t.carrinho) || 0;
      const carrinhoQtde = qtdeCarrinho(transferida, carrinhoPct);
      // A conversão do par origem→destino entra antes do bônus (1 = transferência 1:1).
      // A qtde transferida é o TOTAL da transferência; o carrinho é a parte dela que foi
      // comprada — por isso o carrinho NÃO soma aqui, só reduz o que sai do estoque de origem.
      const fator = Number(t.fator_conversao) > 0 ? Number(t.fator_conversao) : 1;
      const base = Math.round(transferida / fator);
      // Com promoção, a tela manda as milhas de bônus já com o teto aplicado.
      const bonusMilhas = t.bonus_milhas != null
        ? Math.max(0, Math.round(t.bonus_milhas))
        : Math.round(base * (Number(t.bonus) || 0));
      const recebida = base + bonusMilhas;
      const payload = {
        data: t.data,
        ...
        carrinho: carrinhoPct,
        carrinho_qtde: carrinhoQtde,
        bonus_milhas: bonusMilhas,
        fator_conversao: fator,
        promocao_id: t.promocao_id || null,
        qtde_recebida: recebida,
        custo_total: t.custo_total,
        num_parcelas: t.num_parcelas,
        meio_pagamento: t.meio_pagamento ?? "cartao",
        banco_id: t.banco_id ?? null,
...
export function useUpsertProgramaEstoque() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (m: { id?: string; nome: string; ativo?: boolean; tipo?: string | null }) => {
      const payload = { nome: (m.nome || "").trim(), ativo: m.ativo ?? true, tipo: m.tipo || null };
      if (m.id) {
...
export function useProgramasComEstoque() {
  return useQuery({ queryKey: ["programas-com-estoque"], queryFn: async () => {
    const { data, error } = await sb
      .from("programas")
      // "*" traz também as colunas regra_* (aba Regras do cadastro) sem precisar listar uma a uma.
      .select("*, programas_estoque(nome)")
      .order("nome");
    if (error) throw error;
    return (data ?? []) as any[];
  }});
