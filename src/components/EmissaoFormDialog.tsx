...
import { useUpsertEmissao, useClientes, useContas, useCartoes, useProgramas, useOperacoes, useEmissores, useOrigens, useContaProgramas } from "@/hooks/useData";
...
import { vLocalizador, vCodigoLA, vMilhas, vNumPax, vDataVoo } from "@/lib/validacoesEmissao";
import { ajusteVisivel, reaisDiferemDosCobrados, temAlgumAjuste } from "@/lib/ajustesEmissao";
import { Textarea } from "@/components/ui/textarea";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";
import { Send, RefreshCw, Ban, Lock, CheckCircle2, Copy } from "lucide-react";
...
export function EmissaoFormDialog({ open, onOpenChange, editing }: Props) {
  ...
  const programasFiltrados = (programas ?? []).filter((p: any) => !progsDaConta || progsDaConta.has(p.nome));
  const contasFiltradas = (contas ?? []).filter((c: any) => !form.programa || (contaProgramas?.[c.id]?.has(form.programa) ?? false));
  ...
  function validar(): string | null {
    ...
    if (reaisDiferemDosCobrados(form) && !temAlgumAjuste(form))
      return "Os Valores Reais diferem dos Cobrados — preencha ao menos um campo de Ajuste (Cupom, Hack Upgrade, Retarifação, Desconto Promocional ou Campo Aberto).";
  ...
  const dispararN8n = async (acao: "cobrar" | "reprocessar" | "cancelar") => {
    ...
    if (acao === "cancelar" && !confirm("Confirmar CANCELAMENTO desta emissão?")) return;
    setDisparando(acao);
    ...
    try {
      ...
      else if (acao === "cancelar") toast.success(`Emissão ${editing?.id_emissao || ""}: Pix cancelado. Você pode editar, cobrar novamente ou excluir.`, { duration: 8000 });
          ...
          <div className="flex-1 overflow-y-auto pr-2 mt-4 min-h-0">
            ...
            <TabsContent value="geral" className="mt-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="grid gap-2"><Label>Data Voo Ida *</Label><Input type="date" value={form.data_voo_ida} onChange={e => setField("data_voo_ida", e.target.value)} className={vDataVoo(form.data_voo_ida, form.data_emissao) ? "border-destructive" : ""} /><CampoErro msg={vDataVoo(form.data_voo_ida, form.data_emissao)} /></div>
                ...
                {(form.programa || "").toLowerCase() === "latam" && (
                  <>
                    ...
                    <div className="grid gap-2">
                      <Label>Facial Realizada?</Label>
                      <label className="flex items-center gap-2 h-10 px-3 rounded-md border cursor-pointer select-none">
                        <input type="checkbox" className="h-4 w-4 accent-[hsl(var(--primary))]" checked={!!form.facial} onChange={e => setField("facial", e.target.checked)} />
                        <span className="text-sm">{form.facial ? "Sim — pagamento facial" : "Não"}</span>
                      </label>
                    </div>
                  </>
                )}
                ...
                {editing?.id && (
                  <div className="col-span-2 rounded-lg border bg-muted/40 p-3 mt-2">
                    {editing?.status_pix === "PAGO" ? (
                      /* PAGA → nenhum botão, só aviso */
                      <p className="flex items-center gap-1.5 text-sm font-medium text-success">
                        <CheckCircle2 className="h-4 w-4" /> Cobrança paga{editing?.data_recebimento ? ` em ${new Date(editing.data_recebimento).toLocaleString("pt-BR")}` : ""}. Nenhuma ação disponível.
                      </p>
                    ) : editing?.forma_cobranca && editing?.status_pix !== "CANCELADO" ? (
                      /* Cobrada e não paga → Reprocessar + Cancelar */
                      <>
                        <p className="mb-2 flex items-center gap-1.5 text-xs font-medium text-success">
                          <CheckCircle2 className="h-4 w-4" /> Cobrança já emitida{editing?.data_cobranca ? ` em ${new Date(editing.data_cobranca).toLocaleString("pt-BR")}` : ""}. Use "Reprocessar" para reenviar a mesma cobrança por WhatsApp (não gera uma nova).
                        </p>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <Button type="button" variant="outline" className="text-destructive" disabled={disparando !== null} onClick={() => dispararN8n("cancelar")}>
                            <Ban className="h-4 w-4 mr-1" /> {disparando === "cancelar" ? "..." : "Cancelar emissão"}
                          </Button>
                        </div>
                      </>
                    ) : (
                      /* Não cobrada → só Cobrar */
                      <>
                        <p className="mb-2 text-xs font-medium text-muted-foreground">
                          Ações{validar() ? ` — complete os campos antes de cobrar: ${validar()}` : " — emissão pronta para cobrança"}
                        </p>
                        <div className="flex flex-wrap gap-2">
            ...
            <TabsContent value="pix" className="mt-0">
              <div className="space-y-4">
                <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="grid gap-2"><Label>Forma de Cobrança</Label><Input value={editing?.forma_cobranca === "cartao" ? "Cartão" : editing?.forma_cobranca === "pix" ? "Pix" : editing?.forma_cobranca === "link" ? "Link (cliente escolhe)" : "— (não cobrada)"} readOnly disabled /></div>
                    <div className="grid gap-2"><Label>Banco Emissor</Label><Input value={(editing as any)?.pix_banco === "sicredi" ? "Sicredi" : (editing as any)?.pix_banco === "c6" ? "C6 Bank" : "—"} readOnly disabled /></div>
                    <div className="grid gap-2"><Label>Status</Label><Input value={form.status_pix || ""} readOnly disabled /></div>
                    {(editing as any)?.forma_paga && (
                      <div className="grid gap-2"><Label>Pago com</Label><Input value={(editing as any).forma_paga === "cartao" ? "Cartão" : "Pix"} readOnly disabled /></div>
                    )}
                    <div className="grid gap-2"><Label>Valor Recebido</Label><Input value={form.valor_recebido ? `R$ ${Number(form.valor_recebido).toLocaleString("pt-BR",{minimumFractionDigits:2})}` : ""} readOnly disabled /></div>
                    <div className="grid gap-2"><Label>Data Recebimento</Label><Input value={form.data_recebimento || ""} readOnly disabled /></div>
                    <div className="grid gap-2"><Label>ID Emissão</Label><Input value={editing?.id_emissao || (editing ? "" : "Gerado ao salvar")} readOnly disabled /></div>

                    {/* Campos específicos de Pix */}
                    {editing?.forma_cobranca === "pix" && (<>
                      <div className="grid gap-2"><Label>TXID (Pix)</Label><Input value={(editing as any)?.pix_txid || form.txid || ""} readOnly disabled /></div>
                      <div className="col-span-2 grid gap-2"><Label>Pix Copia e Cola</Label><Textarea value={(editing as any)?.pix_copia_cola || ""} readOnly disabled rows={2} className="font-mono text-xs" /></div>
                    </>)}

                    {/* Campos do checkout (cartão ou link de escolha) */}
                    {(editing?.forma_cobranca === "cartao" || editing?.forma_cobranca === "link") && (<>
                      <div className="grid gap-2"><Label>Status do Checkout</Label><Input value={(editing as any)?.checkout_status || ""} readOnly disabled /></div>
                      <div className="grid gap-2"><Label>ID do Checkout</Label><Input value={(editing as any)?.checkout_id || ""} readOnly disabled /></div>
                      <div className="col-span-2 grid gap-2"><Label>Link de Pagamento (Cartão)</Label><Input value={(editing as any)?.checkout_url || ""} readOnly disabled className="font-mono text-xs" /></div>
                    </>)}

                    <div className="col-span-2 grid gap-2"><Label>Observação</Label><Textarea value={form.obs_pix || ""} readOnly disabled rows={2} /></div>
                  </div>
                </div>
              </div>
            </TabsContent>
