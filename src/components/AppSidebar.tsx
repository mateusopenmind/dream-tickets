...
// OBS Dashboard: a Home ("/") é só um desvio (ver `Home` em App.tsx) — manda para o
// Dashboard de Faturamento quem tem acesso a ele, e para /emissoes quem não tem.
// Por isso "Próprias" aponta para /emissoes, e não mais para "/".
...
const groups: MenuGroup[] = [
  {
    label: "Dashboard",
    items: [
      { title: "Faturamento", url: "/dashboard-faturamento", icon: LayoutDashboard, chave: "dashboard_faturamento" },
    ],
  },
  {
    label: "Emissões",
    items: [
      { title: "Próprias", url: "/emissoes", icon: Plane, chave: "emissoes" },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Conferência Pix", url: "/conferencia-pix", icon: ShieldCheck, chave: "conferencia_pix" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { title: "Promoções", url: "/promocoes", icon: Percent, chave: "promocoes" },
...
export function AppSidebar() {
  ...
  // Texto do tooltip exibido quando a sidebar está recolhida (modo ícone).
  // Inclui o grupo para dar contexto, ex.: "Estoque › Compras".
  const rotulo = (label: string, title: string) => (title === label ? title : `${label} › ${title}`);
  ...
  return (
    <Sidebar collapsible="icon">
                  ...
                  <SidebarMenu>
                    {visiveis.map((item) => (
                      <SidebarMenuItem key={item.title}>
                        <SidebarMenuButton asChild tooltip={rotulo(group.label, item.title)}>
      ...
      <SidebarFooter className="p-2">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton onClick={signOut} tooltip="Sair" className="hover:bg-sidebar-accent/50 text-sidebar-foreground/70">
