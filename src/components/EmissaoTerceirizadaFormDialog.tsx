  ...
  const dispararN8n = async (acao: "cobrar" | "reprocessar" | "cancelar") => {
    ...
    if (acao === "cancelar") {
      ...
      try {
        ...
        if (resumo.total > 0) {
          aviso = `\n\n⚠️ Atenção: ${contar(resumo.total, "parcela de recebimento avulso lançada", "parcelas de recebimento avulso lançadas")} para esta emissão` +
            (resumo.recebidos > 0 ? `, sendo ${contar(resumo.recebidos, "já recebida", "já recebidas")} (R$ ${resumo.valor_recebido.toLocaleString("pt-BR", { minimumFractionDigits: 2 })})` : "") +
              ...
              <div className="space-y-4">
                <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="grid gap-2"><Label>Banco Emissor</Label><Input value={ed?.forma_cobranca === "avulso" ? ((ed as any)?.pix_banco || "—") : ({ sicoob: "Sicoob", sicredi: "Sicredi" }[String((ed as any)?.pix_banco || "").toLowerCase()] || (ed as any)?.pix_banco || "—")} readOnly disabled /></div>
