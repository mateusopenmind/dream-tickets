...
import { useUpsertReembolso, useTaxasQueimaCpf, useCartoes, pesquisarEmissoesLocalizador, getEmissaoReembolso, getSaldoPaxReembolso, type SaldoPaxReembolso } from "@/hooks/useData";
...
import { rotuloReembolso } from "@/lib/reembolso";
...
const TIPOS = [
  ...
  { value: "parcial_trecho", label: "Reembolso Parcial 1 Trecho" },
  { value: "taxas", label: "Reembolso Taxas" },
];
// Tipos que reembolsam parte dos pax (têm campo Nº Pax).
const TIPOS_COM_PAX = ["parcial", "parcial_trecho", "taxas"];
const MOTIVOS = [
  { value: "mudanca_planos", label: "Mudança de planos do cliente ou dos passageiros" },
  { value: "atestado_medico", label: "Atestado médico de passageiro" },
...
export function ReembolsoFormDialog({ open = true, onOpenChange, editing, asPage = false }: Props) {
  ...
  const [saldo, setSaldo] = useState<SaldoPaxReembolso | null>(null); // saldo de pax por trecho já reembolsado
  ...
  const temPax = TIPOS_COM_PAX.includes(form.tipo);
  const paxReembolso = temPax ? (Number(form.pax_qtd) || 0) : totalPax;
  const fator = totalPax > 0 ? paxReembolso / totalPax : 1; // proporcional (milhas e taxas)

  // Carrega o saldo de pax já reembolsado da emissão (para limitar o Nº Pax por trecho).
  useEffect(() => {
    if (!em?.id) { setSaldo(null); return; }
    let cancel = false;
    getSaldoPaxReembolso(em.id, em._tabela || "emissoes", editing?.id ?? null)
      .then((s) => { if (!cancel) setSaldo(s); })
      .catch(() => { if (!cancel) setSaldo(null); });
    return () => { cancel = true; };
  }, [em?.id, em?._tabela, editing?.id]);

  // Saldo restante por trecho e o máximo de pax para o tipo/trecho selecionado.
  const Y = saldo?.total_pax ?? totalPax;
  const restanteIda = Math.max(0, Y - (saldo?.ida_usada ?? 0));
  const restanteVolta = Math.max(0, Y - (saldo?.volta_usada ?? 0));
  const restanteTaxas = Math.max(0, Y - (saldo?.taxas_usada ?? 0));
  const maxPax = form.tipo === "parcial" ? Math.min(restanteIda, restanteVolta)
    : form.tipo === "parcial_trecho" ? (form.trecho === "ida" ? restanteIda : form.trecho === "volta" ? restanteVolta : Math.max(restanteIda, restanteVolta))
    : form.tipo === "taxas" ? restanteTaxas
    : totalPax;
  const temSaldoUsado = !!saldo && (saldo.ida_usada > 0 || saldo.volta_usada > 0 || saldo.taxas_usada > 0);

  // Reembolso Taxas não é oferecido quando a cobrança da emissão está EM ABERTO.
  const emEmAberto = em?.status_pix === "EM ABERTO";
  const tiposDisponiveis = TIPOS.filter((t) => !(t.value === "taxas" && emEmAberto));
  // Se a emissão em aberto for selecionada com Taxas ativo, volta para Total.
  useEffect(() => {
    if (emEmAberto && form.tipo === "taxas") setField("tipo", "total");
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [emEmAberto, form.tipo]);
  ...
  useEffect(() => {
    ...
    // Parcial 1 Trecho: só pré-preenche o Preço Milheiro (da emissão); o resto é manual.
    if (form.tipo === "parcial_trecho") {
      setForm((f) => ({ ...f, cli_preco_milheiro: round2(Number(em.preco_milheiro) || 0) }));
      return;
    }
    // Taxas: só taxas (proporcional aos pax); milhas e Queima CPF ficam zeradas.
    const ehTaxas = form.tipo === "taxas";
    const milheiro = Number(em.preco_milheiro) || 0;
    setForm((f) => ({
      ...f,
      // Reembolso Cliente (dos Valores Cobrados)
      cli_milhas: ehTaxas ? 0 : roundInt((Number(em.milhas_cobrado) || 0) * fator),
      cli_preco_milheiro: ehTaxas ? 0 : milheiro,
      ...
      queima_cpf: ehTaxas ? 0 : round2(taxaQueimaValor * paxReembolso),
      // Reembolso Dream Tickets (dos Valores Reais). Custo Milheiro: só existe em terceirizadas.
      dt_milhas: ehTaxas ? 0 : roundInt((Number(em.milhas_real) || 0) * fator),
  ...
  // Parcial 1 Trecho: zera TODOS os valores (nada pré-preenchido, inclusive Queima CPF).
  useEffect(() => {
    if (form.tipo === "taxas") {
      setForm((f) => ({ ...f, cli_milhas: 0, dt_milhas: 0, queima_cpf: 0 }));
    } else if (form.tipo === "parcial_trecho") {
      // Zera tudo, mas preenche o Preço Milheiro com o da emissão.
      setForm((f) => ({
        ...f,
        cli_milhas: 0, cli_preco_milheiro: round2(Number(em?.preco_milheiro) || 0), cli_taxas: 0, cli_bagagens: 0, cli_assentos: 0, cli_outros: 0,
        queima_cpf: 0,
        dt_milhas: 0, dt_custo_milheiro: 0, dt_taxas: 0, dt_bagagens: 0, dt_assentos: 0, dt_outros: 0,
      }));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.tipo]);
  ...
  // RESUMO / liquidação. Base = Reembolso Cliente (brutoCliente). "Pago Emissão" é só informativo
  // (valor recebido da emissão; 0 quando em aberto).
  //  • Emissão PAGA  → sentido "reembolsar": TOTAL = base − Multa − Queima (devolvemos ao cliente).
  //  • Emissão EM ABERTO → sentido "cobrar": TOTAL = base + Multa + Queima (cliente paga tudo, gera Pix).
  // Sentido "a pagar" × "a reembolsar": definido pelo status da emissão na CRIAÇÃO (em aberto → a pagar).
  // Na EDIÇÃO mantém o sentido salvo — pois reembolsar uma emissão em aberto a deixa CANCELADA, e não dá
  // pra recalcular pelo status atual (senão viraria "a reembolsar" por engano, divergindo do WhatsApp).
  const ehAberto = modoEdicao ? ((editing?.sentido ?? "reembolsar") === "cobrar") : (em?.status_pix === "EM ABERTO");
  const pagoEmissao = useMemo(() => (ehAberto ? 0 : round2(Number(em?.valor_recebido) || 0)), [ehAberto, em]);
  const sentido: "reembolsar" | "cobrar" = ehAberto ? "cobrar" : "reembolsar";
  // Base do cálculo ("À Reembolsar/Pagar Emissão"):
  //  • Emissão PAGA (a reembolsar): Total → Pago Emissão (devolve tudo); demais → Reembolso Cliente.
  //  • Emissão EM ABERTO (a pagar): o cliente não pagou nada — ele deve o RESTANTE da emissão:
  //    Total da emissão − Reembolso Cliente (a parte reembolsada sai da conta). No Total, resta 0.
  const emissaoTotal = round2(Number(em?.preco_total ?? editing?.emissao_total) || 0);
  const baseCalc = ehAberto
    ? (form.tipo === "total" ? 0 : round2(Math.max(0, emissaoTotal - brutoCliente)))
    : (form.tipo === "total" ? pagoEmissao : brutoCliente);
  const totalLiquidacao = useMemo(() => {
    const multa = Number(form.multa_programa) || 0;
    const queima = Number(form.queima_cpf) || 0;
    return ehAberto
      ? round2(-(baseCalc + multa + queima))   // a pagar (negativo)
      : round2(baseCalc - multa - queima);      // a reembolsar (positivo)
  }, [ehAberto, baseCalc, form.multa_programa, form.queima_cpf]);
  ...
  function validar(): string | null {
    ...
    if (form.tipo === "parcial_trecho" && !form.trecho) return "Selecione o Trecho (ida ou volta).";
    if (form.tipo === "total" && temSaldoUsado) return "Esta emissão já tem reembolsos parciais; não é possível fazer um Reembolso Total.";
    if (temPax) {
      if (!form.pax_qtd || form.pax_qtd < 1) return "Informe o Nº de Pax do reembolso.";
      if (form.pax_qtd > maxPax) return `Sem saldo: o Nº de Pax não pode passar de ${Math.max(0, maxPax)} para este ${form.tipo === "parcial_trecho" ? "trecho" : "tipo"}.`;
    }
    return null;
  }
  ...
  async function executarSalvar() {
    ...
    setSalvando(true);
    // Cobrar com liquidacao ZERADA nao tem Pix a gerar (nao existe cobranca de R$ 0,00).
    // Nesse caso segue pelo resumo por WhatsApp, igual ao lado de reembolsar.
    const cobrar = sentido === "cobrar" && Number(totalLiquidacao || 0) !== 0;
    ...
    try {
      const criado: any = await upsert.mutateAsync({
        ...
        tipo: form.tipo,
        trecho: form.tipo === "parcial_trecho" ? (form.trecho || null) : null,
        motivo: form.motivo || null,
        pax_qtd: temPax ? Number(form.pax_qtd) : totalPax,
        pax_total: totalPax,
        ...
        // resumo / liquidação
        valor_pago_emissao: pagoEmissao,
  ...
  async function reenviarWhatsapp() {
    if (!editing?.id) { toast.error("Salve o reembolso antes."); return; }
    // Zerado nunca tem Pix: vai pelo resumo padrao.
    const valorLiq = Number(rb?.total_liquidacao ?? editing?.total_liquidacao ?? totalLiquidacao ?? 0);
    const ehCobrar = (rb?.sentido ?? editing?.sentido ?? sentido) === "cobrar" && valorLiq !== 0;
    // "a cobrar" com Pix já gerado → reprocessa (reenvia o mesmo Pix). Sem Pix → gera o Pix agora.
    const temPix = !!(rb?.pix_copia_cola ?? editing?.pix_copia_cola) || !!(rb?.pix_txid ?? editing?.pix_txid);
    setDisparando("reenviar");
    const t = toast.loading("Reenviando WhatsApp...");
    try {
      const acao = ehCobrar ? (temPix ? "reprocessar" : "cobrar") : "reembolso_resumo";
      const body: any = acao === "cobrar"
        ? { acao, forma: "pix", emissao_id: editing.id, tabela: "reembolsos" }
        : { acao, emissao_id: editing.id, tabela: "reembolsos" };
      const { data, error } = await supabase.functions.invoke("disparar-n8n", { body });
  ...
  const tipoResumoLabel = rotuloReembolso(form.tipo, paxReembolso, totalPax);
          ...
          {modoEdicao && rb && rb.sentido === "cobrar" && (
            <div className="rounded-lg border p-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
                ...
                <Info label="Banco" value={({ sicoob: "Sicoob", sicredi: "Sicredi" }[String(rb.pix_banco || "").toLowerCase()] || rb.pix_banco || "—")} />
              </div>
              {rb.pix_copia_cola && (
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  {form.tipo === "parcial_trecho" && (
                    <div className="grid gap-1.5 mt-1">
                      <Label>Trecho *</Label>
                      <div className="inline-flex rounded-md border overflow-hidden text-sm w-fit">
                        {(["ida", "volta"] as const).map((tr) => (
                          <button key={tr} type="button" onClick={() => setField("trecho", tr)}
                            className={`px-4 py-2 capitalize transition-colors ${form.trecho === tr ? "bg-primary text-primary-foreground font-semibold" : "bg-background text-muted-foreground hover:bg-muted"}`}>
                            {tr}
                          </button>
                        ))}
                      </div>
                  ...
                  {temPax && (
                    <div className="grid gap-1.5 mt-1">
                      <Label>Nº Pax (máx. {Math.max(1, maxPax)}) *</Label>
                      <Input type="number" min={1} max={Math.max(1, maxPax)} value={form.pax_qtd}
                        onChange={(e) => setField("pax_qtd", Math.min(Math.max(1, maxPax), Math.max(0, parseInt(e.target.value) || 0)))} />
                      {maxPax <= 0 && (
                        <p className="text-[11px] text-destructive">Sem saldo de pax para este {form.tipo === "parcial_trecho" ? "trecho" : "tipo"} nesta emissão.</p>
                      )}
                    </div>
                  )}
                  {temSaldoUsado && (
                    <p className="text-[11px] text-muted-foreground mt-1">
                      Saldo desta emissão — Ida: {restanteIda}/{Y} · Volta: {restanteVolta}/{Y} · Taxas: {restanteTaxas}/{Y} disponíveis.
                    </p>
                  )}
                </div>
                <div className="grid gap-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="grid gap-2"><Label>Qtde Milhas{form.tipo === "parcial" ? " (proporcional)" : ""}</Label><NumericInput value={form.cli_milhas} onChange={(n) => setField("cli_milhas", n)} /></div>
                  <div className="grid gap-2"><Label>Preço Milheiro</Label><NumericInput value={form.cli_preco_milheiro} onChange={(n) => setField("cli_preco_milheiro", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Taxas{form.tipo === "parcial" || form.tipo === "taxas" ? " (proporcional)" : ""}</Label><NumericInput value={form.cli_taxas} onChange={(n) => setField("cli_taxas", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Bagagens</Label><NumericInput value={form.cli_bagagens} onChange={(n) => setField("cli_bagagens", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Assentos</Label><NumericInput value={form.cli_assentos} onChange={(n) => setField("cli_assentos", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Outros</Label><NumericInput value={form.cli_outros} onChange={(n) => setField("cli_outros", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Multa Programa</Label><NumericInput value={form.multa_programa} onChange={(n) => setField("multa_programa", n)} decimal prefix="R$" /></div>
                  ...
                  <div className="grid gap-2">
                    ...
                    <NumericInput value={form.queima_cpf} onChange={(n) => setField("queima_cpf", n)} decimal prefix="R$" />
                    {taxaQueimaValor > 0 && (
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="grid gap-2"><Label>Qtde Milhas{form.tipo === "parcial" ? " (proporcional)" : ""}</Label><NumericInput value={form.dt_milhas} onChange={(n) => setField("dt_milhas", n)} /></div>
                  <div className="grid gap-2"><Label>Custo Milheiro</Label><NumericInput value={form.dt_custo_milheiro} onChange={(n) => setField("dt_custo_milheiro", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Taxas{form.tipo === "parcial" ? " (proporcional)" : ""}</Label><NumericInput value={form.dt_taxas} onChange={(n) => setField("dt_taxas", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Bagagens</Label><NumericInput value={form.dt_bagagens} onChange={(n) => setField("dt_bagagens", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Assentos</Label><NumericInput value={form.dt_assentos} onChange={(n) => setField("dt_assentos", n)} decimal prefix="R$" /></div>
                  <div className="grid gap-2"><Label>Outros</Label><NumericInput value={form.dt_outros} onChange={(n) => setField("dt_outros", n)} decimal prefix="R$" /></div>
                </div>
                <div className="mt-2 flex justify-end">
                ...
                <div className="rounded-md border divide-y bg-background">
                  {form.tipo === "total" ? (
                    <div className="flex justify-between px-3 py-1.5 text-sm">
                      <span className="text-muted-foreground">Pago Emissão</span>
                      <span className="font-medium">{fmtMoeda(pagoEmissao)}</span>
                    </div>
                  ) : form.tipo === "taxas" ? (
                    <div className="flex justify-between px-3 py-1.5 text-sm">
                      <span className="text-muted-foreground">{sentido === "reembolsar" ? "À Reembolsar Emissão" : "(-) À Pagar Emissão"}</span>
                      <span className="font-medium">{fmtMoeda(baseCalc)}</span>
                    </div>
                  ) : (
                    <>
                      <div className="flex justify-between px-3 py-1.5 text-sm">
                  ...
                  <div className={`flex justify-between px-3 py-2 text-sm font-semibold ${sentido === "reembolsar" ? "text-emerald-700 bg-emerald-50" : "text-rose-700 bg-rose-50"}`}>
                    <span>{sentido === "reembolsar" ? "TOTAL À REEMBOLSAR" : "TOTAL À PAGAR"}</span>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="rounded-lg border divide-y">
              ...
              {form.tipo === "total" ? (
                <div className="flex justify-between px-3 py-1.5"><span className="text-muted-foreground">Pago Emissão</span><span className="font-medium">{fmtMoeda(pagoEmissao)}</span></div>
              ) : form.tipo === "taxas" ? (
                <div className="flex justify-between px-3 py-1.5"><span className="text-muted-foreground">{sentido === "reembolsar" ? "À Reembolsar Emissão" : "(-) À Pagar Emissão"}</span><span className="font-medium">{fmtMoeda(baseCalc)}</span></div>
              ) : (
                <>
                  <div className="flex justify-between px-3 py-1.5"><span className="text-muted-foreground">Pago Emissão</span><span className="font-medium">{fmtMoeda(pagoEmissao)}</span></div>
                  <div className="flex justify-between px-3 py-1.5"><span className="text-muted-foreground">{sentido === "reembolsar" ? "À Reembolsar Emissão" : "(-) À Pagar Emissão"}</span><span className="font-medium">{fmtMoeda(baseCalc)}</span></div>
              ...
              <div className="flex justify-between px-3 py-1.5"><span className="text-muted-foreground">Queima de CPF</span><span className="font-medium text-destructive">(-) {fmtMoeda(form.queima_cpf)}</span></div>
